...
SELECT DISTINCT CR.ContactID, 'Active' as IsActive FROM Tags T (NOLOCK)
	...
	join CampaignRecipients CR (NOLOCK) ON CR.ContactId = C.ContactID AND CR.AccountID = C.AccountID
	join CampaignStatistics CS (NOLOCK) ON CR.CampaignRecipientID = CS.CampaignRecipientID AND CS.AccountID = CR.AccountID and CS.ActivityDate < @tagsActive
