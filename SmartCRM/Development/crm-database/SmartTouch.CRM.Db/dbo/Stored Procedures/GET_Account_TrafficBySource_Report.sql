...
CREATE PROC [dbo].[GET_Account_TrafficBySource_Report]
	(
		@AccountID			int,
		@StartDate          datetime,
		@EndDate            datetime,
	...
	BEGIN TRY
		...
		SELECT	@LeadSourceList = COALESCE(@LeadSourceList + ', ', '') + '[' + DropdownValue + ']',
			...
			FROM( SELECT DISTINCT DropdownValue FROM dbo.DropdownValues(NOLOCK)
					WHERE DropdownID = 5 AND (AccountID = @AccountID OR AccountID IS NULL)
				) Temp
						...
						FROM	(
									...
									FROM	(
												...
												FROM	dbo.Users(NOLOCK) U INNER JOIN DBO.DropdownValues DV ON U.AccountID = DV.AccountID
														INNER JOIN dbo.Contacts(NOLOCK) C ON C.AccountID = U.AccountID AND C.OwnerID = U.UserID
														INNER JOIN dbo.ContactLeadSourceMap(NOLOCK) CSM ON CSM.ContactID = C.ContactID AND CSM.LeadSouceID = DV.DropdownValueID
...
/*
	  EXEC [dbo].[GET_Account_TrafficBySource_Report]
		 @AccountID			=  45,
		 @StartDate         = '2014-02-17 10:55:19.757', 
		 @EndDate           = '2014-11-25 10:42:50.827',      
		 @UserIDs			= '11,49,50,51,64,65', 
		 @CommunityIDs		= ''
