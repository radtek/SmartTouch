	...
	BEGIN
		...
		ELSE IF @LeadScoreConditionType = 3 OR @LeadScoreConditionType= 1 -- submits form or opens email
			BEGIN
				...
				SET @S_EntityId = @EntityId
			END
		-- contact action tag added/not tag added/lead source added/tour type/note category type added/ action type added
		ELSE IF @LeadScoreConditionType = 6 OR @LeadScoreConditionType =7 OR @LeadScoreConditionType = 8 OR @LeadScoreConditionType = 9  OR @LeadScoreConditionType = 27 OR @LeadScoreConditionType = 30
			BEGIN
		...
		IF @RuleCount > 0
			BEGIN
				...
				INSERT INTO IndexData
				SELECT NEWID(), @ContactID, 1, GETUTCDATE(), NULL, 1,1
			END
		ELSE
			BEGIN
				-- update message
