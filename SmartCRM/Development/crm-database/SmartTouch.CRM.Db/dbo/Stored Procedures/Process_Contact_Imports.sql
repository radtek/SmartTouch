...
BEGIN
	...
	BEGIN
		...
		ELSE
			BEGIN
				UPDATE ICD
				SET ICD.LoopID	= 0 , ValidEmail = dbo.EmailValidation(ICD.PrimaryEmail, ICD.FirstName, ICD.LastName)
				FROM dbo.ImportContactData ICD WHERE ICD.IsDuplicate = 1 AND JobId = @leadAdapterJobLogID
			END
			 ...
			 BEGIN 
					...
					SET @DropDownValueID = (SELECT TOP 1 DropDownValueID  FROM Dropdownvalues  (NOLOCK) where  DropDownID = 5 AND  AccountID = @AccountID and [DropdownValue] = @Value )
					 ...
					 IF (@DropDownValueID IS NULL AND  lower(@Value) != lower('BDX-BDX') )
					  ...
					  IF (@DropDownValueID IS NOT NULL AND  lower(@Value) != lower('BDX-BDX') 
		...
		BEGIN
			...
			while exists(select 1 from @dupIdList)
			 begin
				...
				select LeadAdapterAndAccountMapID,StartDate,EndDate,1,Remarks,FileName,CreatedBy,DATEADD(MINUTE,@dupCounter*5, GETUTCDATE()) ,StorageName from LeadAdapterJobLogs (NOLOCK) 
