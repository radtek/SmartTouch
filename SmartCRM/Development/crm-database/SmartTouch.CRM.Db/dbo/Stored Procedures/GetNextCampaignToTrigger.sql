...
BEGIN
	...
	ELSE 
		BEGIN	
			...
			DECLARE @QualifiedEndTime DATETIME = DATEADD(MINUTE,-30,GETUTCDATE())
			print @QualifiedStartTime
			...
			LastRetry (CampaignID,RetriedOn,CampaignRetryAuditID)
			AS
			(
				SELECT CampaignID,MAX(RetriedOn) RetriedOn,Max(CampaignRetryAuditID) CampaignRetryAuditID FROM CampaignRetryAudit (NOLOCK)
					GROUP BY CampaignID
			), 
			...
			( 
				...
				WHERE C.IsDeleted = 0 AND C.CampaignStatusID = @Delayed AND c.IsRecipientsProcessed = 1 AND C.ScheduleTime > @QualifiedStartTime
			)	
			SELECT * INTO #TTable FROM Camps
				WHERE (RetriedOn <= @QualifiedEndTime OR RetriedOn IS NULL)
