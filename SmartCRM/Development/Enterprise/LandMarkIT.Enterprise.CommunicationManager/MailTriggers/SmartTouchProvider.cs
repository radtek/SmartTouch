...
namespace LandmarkIT.Enterprise.CommunicationManager.MailTriggers
{
    ...
    {
        public int BatchCount { get { return 5000; } }
            ...
            , string providerEmail, MailRegistrationDb mailRegistration)
        {
            ...
            //SubmitWithTimeOut(connection, message, campaign.Id, campaign.Subject); // commented by ram on 11th may 2018 for ticket NEXG-3007 NEXG-3004
            bool _connectionstatus = SubmitWithTimeOut_testing(mailRegistration, message, campaign.Id, campaign.Subject);
            for (int i = 1; i <= 2; i++)
            {
                ...
                {
                    int milliseconds = i* 5000;
                    System.Threading.Thread.Sleep(milliseconds);
                    _connectionstatus = SubmitWithTimeOut_testing(mailRegistration, message, campaign.Id, campaign.Subject);
                }
            }
        ...
        public bool SubmitWithTimeOut_testing(MailRegistrationDb mailRegistration, Message message, int campaignId, string subject) // created this method for testing PMTA emails by ram on 11th may 2018 for NEXG-3007 NEXG-3004
        {
            ...
            {
                Logger.Current.Critical("PMTA Connection failed for Job Id : " + message.JobID + " Error Message : ", _ex);
                _connectionStatus = false;
                connection.Close();
            }
            finally
