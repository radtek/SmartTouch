...
namespace LandmarkIT.Enterprise.Utilities.Excel
{
    public class ReadExcel
    {
        ...
        public DataSet ToDataSetImport(string filePath, bool isFirstRowAsColumnNames)
        {
            try
            {
                Logger.Current.Verbose("Get Rows content for the uploaded excel file");
                var fileInfo = new FileInfo(filePath);
                var stream = File.Open(filePath, FileMode.Open, FileAccess.Read);
                var excelReader = (string.Equals(".xls", fileInfo.Extension, StringComparison.OrdinalIgnoreCase)) ? ExcelReaderFactory.CreateBinaryReader(stream) 
