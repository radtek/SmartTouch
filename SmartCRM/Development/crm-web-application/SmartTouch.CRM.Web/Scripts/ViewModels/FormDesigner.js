var formViewModel = function (data, webApp, webService, accountId, userId) {
    ...
    setTimeout(function () {
        if (initialLoad) {
            ...
            //selfForm.FormFields().filter(function (e) { return e.FieldId() == 74 })[0].Value($(data.HTMLContent).find("#74").val());
        ...
        read: function () {
            ...
            ko.utils.arrayForEach(selfDesigner.Controls(), function (item) {
                if (item.EditedHTML() !== "") {
                    ...
                    if (item.FieldId() == 74) {
                        document.getElementById("74").value = item.Value();
                        $(temp).find('#74').val(item.Value());
                    }
                    cloned = $(temp).clone();
            ...
            viewableHtml = viewableHtml + htmlFormTag
                ...
                + "}"
                + "setTimeout(function(){"
                + "$(document).bind('keyup keypress', function(e) {var keyCode = e.keyCode || e.which; if(keyCode==13){$('#" + submitformid + "').find('input[type=" + '"submit"' + "]').click();}});"
                ...
                + "$('#submitform" + formId + "').submit(function (e) {e.preventDefault();"
