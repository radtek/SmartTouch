var TourViewModel = function (data, url, isEditView,notselectall) {
    ...
    selfTour.IcsCalendarToContact = ko.observable(false);
    ...
    selfTour.LifeCycleStages = ko.observable(data.LifeCycleStages);
    selfTour.LifeCycleStage = ko.observable(data.LifeCycleStage);
    ...
    selfTour.LifeCycleCounter = ko.observable(0);
    selfTour.Contacts = ko.observableArray(data.Contacts);//.extend({ minimumLength: 1 });
    selfTour.Contacts.subscribe(function () {
        var Ids = selfTour.Contacts().map(function (e) { return e.Id; });
        if (Ids)
            Ids = ko.toJSON(Ids);
        if ((data.LifeCycleStage == 0 || data.LifeCycleStage == 'undefined' || data.LifeCycleStage == null) && selfTour.TourID() > 0) {
            setTimeout(function () {
                var dropdownlist = $("#TourLifeCycleStage").data("kendoDropDownList");
                if (dropdownlist)
                    dropdownlist.enable(false);
            }, 1000);
        }

        if (selfTour.TourID() == 0 || (selfTour.TourID() > 0 && selfTour.LifeCycleCounter() > 0)) {
            $.ajax({
                url: url + 'GetLifeCycleStageForTour',
                type: 'post',
                dataType: 'json',
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify({ 'contactIds': Ids })
                ...
                var counter = selfTour.LifeCycleCounter();
                selfTour.LifeCycleCounter(counter + 1);
                selfTour.LifeCycleStage(response.response);
                console.log("LC : " + response.response);
                if (selfTour.LifeCycleStage() == 0) {
                    var dropdownlist = $("#TourLifeCycleStage").data("kendoDropDownList");
                    dropdownlist.enable(false);
                } else {
                    setTimeout(function () { 
    ...
    selfTour.allowIcsCalenderToContact = function (value) {
        if (value.IcsCalendarToContact() === true)
            selfTour.IcsCalendarToContact(true);
        else
            selfTour.IcsCalendarToContact(false);
    }
