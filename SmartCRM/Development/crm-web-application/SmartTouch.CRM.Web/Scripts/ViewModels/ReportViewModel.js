var opportunityPipelineViewModel = function (url, data, users, dateFormat, Type, runReport, itemsPerPage, Activities, module, filename) {
    ...
    if (Type == "C") {
        selflist.Groups = ko.observableArray([
                   { GroupId: 0, Group: '[|No Filter|]' },
                   { GroupId: 1, Group: '[|Account Executive|]' },
                   { GroupId: 2, Group: '[|Community|]' }
        ]);
    }
    else {
        selflist.Groups = ko.observableArray([
    ...
    selflist.ActivityModule = ko.observable();
    ...
    var bothtourTypesAnaLC = selflist.LifecycleStages().concat(selflist.TourTypes());
    selflist.BothResults = ko.observableArray(bothtourTypesAnaLC);
    ...
    selflist.accountExecVisible = ko.observable(Type == "C" ? "False" : "True");
    selflist.communityVisible = ko.observable(Type == "C" ? "True" : "False");
    ...
    selflist.periodChange = function () {
        ...
        if (value == "1")  // last 7 days
            ...
            selflist.CustomStartDate(moment(minDate).format());
        }
        if (value == "7") { // custom
    ...
    function CreateGrid(data) {
        ...
        if (Type == 'C') {
            ...
            if (selflist.GroupId() == 0 || selflist.GroupId() == 2)
                columns.push({
                    field: entryIndex,
                    title: "[|Community|]",
                    footerTemplate: "[|Total|]"
            ...
            if (i == data[0].DropdownValues.length - 1) {
                ...
                if (selflist.IsCompared() == true) {
                    ...
                    columns.push({
                        ...
                        footerTemplate: Sum(seq, "CurrentTotal") == "0" ? "0" : "<a href='javascript:void(0)' onClick=myCustomFunction(" + "'" + curruntColumnName + "'" + "," + "'" + data[0].DropdownValues[i].DropdownType + "'" + ")>" + Sum(seq, "CurrentTotal") + "</a>"
                    });
                    ...
                    columns.push({
                        ...
                        footerTemplate: Sum(seq, "PreviousTotal") == "0" ? "0" : "<a href='javascript:void(0)' onClick=myCustomFunction(" + "'" + previousColumnName + "'" + "," + "'" + data[0].DropdownValues[i].DropdownType + "'" + ")>" + Sum(seq, "PreviousTotal") + "</a>"
                    });
                }
                    ...
                    columns.push({
                        field: entryIndex,
                        title: columnName,
                        footerTemplate: Sum(seq, "Total") == "0" ? "0" : "<a href='javascript:void(0)' onClick=myCustomFunction(" + "'" + columnName.replace(/\s+/g, "") + "'" + "," + "'" + data[0].DropdownValues[i].DropdownType + "'" + ")>" + Sum(seq, "Total") + "</a>"
                    });
                }
            }
        ...
        var showing = null;
        if (Type == 'C') {
            showing = (selflist.GroupId() == 1 ? '[|Account Executives|]' : '[|Communities|]')
        }
        else {
            showing = (selflist.GroupId() == 0 ? '[|Account Executives|]' : '[|Communities|]')
        }
        ...
        var configuration = {
            ...
            pageable: {
                ...
                messages: {
                    display: "[|Showing|] {0}-{1} [|from|] {2} " + showing
                },
            },
            columns: columns,
            sortable: true,
                ...
                $('td').each(function (index, value) {
                    if (index >= colLength) {
                        ...
                        if (Type == 'T')
                            ...
                            value = ko.utils.arrayFirst(selflist.BothResults(), function (item) {
                                if (selflist.IsCompared() == 1)
                                    dropdownvalueid = dropdownvalueid.replace("(c)", "").replace("(p)", "");
                                return item.DropdownValue.toString() === dropdownvalueid;
                            });
                        ...
                        if (!isNaN(dropdowns[0]) && dropdowns[0] != undefined && parseInt(dropdowns[0]) > 0) {
                            var groupByValue = [];
                            data.map(function (v) { groupByValue.push({ name: v.Name, Id: v.ID, Dropdownvalues: v.DropdownValues }) });
                            ...
                            if (Type == "C") {
                                if (row != null && row.Dropdownvalues.length > 0) {
                                    DropdownType = ko.utils.arrayFirst(row.Dropdownvalues, function (d) {
                                        if (selflist.IsCompared() == 1) {
                                            d.DropdownValueName = d.DropdownValueName.replace("(c)", "").replace("(p)", "");
                                        }

                                        if (d.DropdownValueName == dropdownvalueid)
                                            return d;
                                    });
                                }
                            }
                            ...
                            if (Type == "C" && row != null && DropdownType != null)
                                anchor.setAttribute("rowtype", DropdownType.DropdownType);
                            else if (Type == "C" && (dropdownvalueid == "Total Count" || selflist.IsCompared() == 1))
                                anchor.setAttribute("rowtype", "LC");
                            ...
                            if ((Type == 'A' && (dropdownvalueid == 'Contacts' || dropdownvalueid == 'Total')) || Type != 'A') {
                                anchor.id = selectedcell;
                                ...
                                anchor.onclick = function () {
                                    selflist.RowId(parseInt(this.getAttribute("rowid")));
                                    selflist.ActivityModule(this.getAttribute("Module"));
                                    selflist.DropdownType(this.getAttribute("rowtype"));
