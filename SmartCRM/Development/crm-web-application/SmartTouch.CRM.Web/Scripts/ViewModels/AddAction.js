...
var ActionViewModel = function (data, url, reminderTypes, _dateformat, notselectall, IsIncludeSignature, actionMode) {
    ...
    selfAction.IcsCanlenderToContact = ko.observable(false);
    ...
    selfAction.allowIcsCalenderToContact = function(value){
        if (value.IcsCanlenderToContact() === true)
            selfAction.IcsCanlenderToContact(true);
        else
            selfAction.IcsCanlenderToContact(false);
    }
    ...
    selfAction.reminderDateValidation = selfAction.RemindOnDate.extend({
        validation: {
            validator: function (val) {
                if (val >= selfAction.ActionStartTimeOn() && selfAction.SelectedReminderTypes.length > 0)
                    return false;
                else
                    return true;
            },
            message: '[|Remind On time should be less than ActionStartTime|]'
        }
    });
