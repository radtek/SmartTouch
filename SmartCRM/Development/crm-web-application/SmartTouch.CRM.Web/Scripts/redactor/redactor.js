...
RedactorPlugins.imagemanager = function () {
    return {
        ...
        load: function () {
            ...
            this.modal.addTab(2, 'Image Gallery');

            $('#redactor-modal-image-droparea').addClass('redactor-tab redactor-tab1');

            var $box = $('<div id="redactor-image-manager-box" style="overflow: scroll; height: 300px;" class="redactor-tab redactor-tab2" data-title="Choose">').hide();
            this.modal.getModal().append($box);

            $("#redactor-image-manager-box").scroll(function () {
            ...
            loadImages = function (image) {
                ...
                if (hasData) {
                    $.ajax({
                        dataType: "json",
                        cache: false,
                        url: image.opts.imageManagerJson,
                        data: { 'PageNumber': pageNumber },
                        success: $.proxy(function (data) {
                            var imagesReceived = data.Data || data.data;
                            var validImages = false;
                            if (imagesReceived) {
                                if (imagesReceived.length == 0)
                                    hasData = false;
                                $.each(imagesReceived, $.proxy(function (key, val) {
                                    if (images.indexOf(val.ImageID) < 0) {
                                        validImages = true;
                                        var thumbtitle = '';
                                        if (typeof val.FriendlyName !== 'undefined') {
                                            thumbtitle = val.FriendlyName;
                                        }
                                        var img = $('<img class="lazy redactor-image-gallery" src="' + val.StorageName + '"  data-params="' + encodeURI(JSON.stringify(val)) + '" style="width: 100px; height: 75px; padding: 2px; cursor: pointer;" data-title="' + thumbtitle + '"/>');
                                        $box.append(img);
                                        $(img).click($.proxy(image.imagemanager.insert, image));
        ...
        insert: function (e) {
            ...
            this.image.insert('<img src="' + $(e.target).attr('src') + '" alt="' + $(e.target).attr('data-title') + '" width="150" height="auto">');
...
RedactorPlugins.table = function () {
    return {
        ...
        insert: function () {
            ...
            if (applyTableResizable) applyTableResizable();
        },
        getTable: function () {
...
RedactorPlugins.fullscreen = function () {
    return {
        ...
        enable: function () {
            ...
            this.$editor.css('max-height', '960px');
        ...
        disable: function () {
            ...
            this.$editor.css('max-height', '130px');
...
RedactorPlugins.fontsize = function () {
    return {
        init: function () {
            var fonts = [12, 14, 16, 18, 20, 24, 32];
            var that = this;
            var dropdown = {};
...
RedactorPlugins.savecode = function () {
    return {
        save: function () {
            ...
            this.core.destroy();
            $('.sce-redactor-init').removeClass('sce-redactor-init');
            //$('.scp-container .scp-panel').sortable('enable').disableSelection();
            //console.log(html);
        }
    };
};
...
RedactorPlugins.advanced = {
    init: function () {
        this.buttonAdd('advanced', 'Save', this.testButton);
    },
    testButton: function (buttonName, buttonDOM, buttonObj, e) {
        //alert($(e.target).hasClass('re-advanced'));
        $(e.target).parents('div.redactor_box').find('.scw-content').redactor('destroy');
    }
};
...
RedactorPlugins.clips = function () {
    return {
        init: function () {
            var clipOptions = this.opts.clipsManager;
            if (clipOptions.length) {

                this.clips.template = $('<ul id="redactor-modal-list">');
                for (var i = 0; i < clipOptions.length; i++) {
                    var li = $('<li>');
                    var a = $('<a href="#" class="redactor-clip-link">').text(clipOptions[i][0]);
                    var div = $('<div class="redactor-clip">').hide().html(clipOptions[i][1]);
                    li.append(a);
                    li.append(div);
            ...
            var button = this.button.add('clips', 'Placeholders');
            this.button.addCallback(button, this.clips.show);
        },
        show: function () {
            this.modal.load('clips', 'Insert Placeholders', 400);
            this.modal.createCancelButton();
...
//Campaign template plugin
if (!RedactorPlugins) var RedactorPlugins = {};
RedactorPlugins.templates = function () {
    return {
        getTemplate: function () {
            return String()
        ...
        init: function () {
            this.modal.addTemplate('templates', this.templates.getTemplate());

            var button = this.button.add('templates', 'Choose a template');

            //this.button.addCallback(button, this.templates.show);
        },
        show: function () {
            //this.modal.load('templates', 'Choose a template', 800);
            //var $modal = this.modal.getModal();

            //this.modal.createTabber($modal);
            //this.modal.addTab(1, 'Predefined Templates', 'active');
            //this.modal.addTab(2, 'Saved Templates');
            //this.modal.addTab(2, 'Sent Campaigns');
            //this.modal.show();
        },
        load: function () {
        ...
        button: function () {
            return {
                build: function (btnName, btnObject) {
                    var $button;
                    if (btnName == "templates")
                        $button = $('<a data-target="#sendemailmodal" data-toggle="modal" href="#" class="re-icon re-' + btnName + '" rel="' + btnName + '" onclick=" return templatesModal();"/>');
                    else
                        $button = $('<a href="#" class="re-icon re-' + btnName + '" rel="' + btnName + '" />');
        ...
        dropdown: function () {
            return {
                ...
                show: function (e, key) {
                    ...
                    //$dropdown.on('mouseover', function () { $('html').css({ 'overflow': 'hidden' ,'margin-right':'8px'}); });
                    //$dropdown.on('mouseout', function () { $('html').css({ 'overflow': '', 'margin-right': '0px' }); });
                ...
                setVerified: function (html) {
                    ...
                    if (matches) {
                        ...
                        for (var i = 0; i < len; i++) {
                            ...
                            html = html.replace(new RegExp(matches[i].replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"), 'gi'), newTag);
                        }
                    }
            ...
            return {
                show: function (e) {
                    ...
                    if (selfCampaign) {
                        ...
                        $('<label><input type="checkbox" id="url-mergefields"/> Include merge fields in URL</label>').insertAfter($('#redactor-link-url-text'));

                        $('#url-mergefields').click(function () {
                            if (this.checked == true) {

                                $('<div class="form-group medium"><label class="control-label">[|Fields|] </label><input id="mergefields" /></div><div class="form-group medium"><a class="btn btn-lg btn-primary" id="btn-appendToURL">Add Field</a></div>').insertAfter($('#redactor-link-url-text'));
                                ...
                                $("#btn-appendToURL").on('click', function () {
                                    if (fieldCode) {
                                        var $txt = $('#redactor-link-url');
                                        var caretPos = $txt[0].selectionStart;
                                        var textAreaTxt = $txt.val();
                                        var txtToAdd = "*|" + fieldCode + "|*";
                                        $txt.val(textAreaTxt.substring(0, caretPos) + txtToAdd + textAreaTxt.substring(caretPos));
                                    }
                                    else {
                                        $("#mergefield-validation").show();
                                        setTimeout(function () {
                                            $("#mergefield-validation").css({ 'display': 'none' });
                                        }, 5000);
                                    }
                                });
                                $('.k-dropdown-wrap.k-state-default').click(function () {
                                    $("#mergefield-validation").hide();
                                })
                                $('<span class="validationMessage" id="mergefield-validation" style="display:none">Please select a field</span>').insertAfter($('#redactor-modal-link-insert .k-widget.k-dropdown.k-header'))
                                $('#redactor-modal-link-insert').height(240);

                            }
                            $('#url-mergefields').parent().remove();

                        })
                        var openInNewTab = $('#redactor-link-blank')[0].checked;
                        $('#redactor-link-blank').parent().remove();
                        var redactorLinkBlank = '<label class="pullLabel-right" style="display: inline-block;float :right!important"><input type="checkbox" id="redactor-link-blank"> Open link in new tab</label>';
                        $(redactorLinkBlank).insertAfter($('#redactor-modal-link-insert label').first());
                        $('#redactor-link-blank')[0].checked = openInNewTab;
                        $('#redactor-modal label').first().css({ 'display': 'inline-block' });
                        $('#redactor-link-blank').parent().css({ 'display': 'inline-block' });
                        $('#redactor-link-blank').parent().addClass('pullLabel-right');
                    }
            ...
            return {
                ...
                showEdit: function ($image) {
                    ...
                    if (selfCampaign) {
                        ...
                        $('<label id="label-mergefields"><input type="checkbox" id="url-mergefields"/> Include merge fields  in URL</label>').insertAfter($('#redactor-image-align'));

                        var imageWidth = ($($image[0].outerHTML).width() ? $($image[0].outerHTML).width() : 0) || $image.width();
                        var imageHeight = ($($image[0].outerHTML).height() ? $($image[0].outerHTML).height() : 0) || $image.height();
                        $('<div><span class="campaign_lock"><label> Width : </label> <input type="text" id="redactor_image_width" maxlength="5" style="width:12%" />' +
                            '<span id="checkbox-image" class="checkbox-image"></span>' +
                            //'<label for="cbox2">Lock</label> <input type="checkbox" id="cbox2"/>' +
                            '<label> Height : </label><input type="text" id="redactor_image_height" style="width:12%"  maxlength="5" /></span></div>')
                            .insertAfter('#label-mergefields');
                        if (imageWidth)
                            document.getElementById("redactor_image_width").value = imageWidth;
                        //console.log("1 image height: " + imageHeight);
                        if (imageHeight) {
                            // console.log("2 image height: " + imageHeight);
                            document.getElementById("redactor_image_height").value = imageHeight;
                        }
                        var count = 0;
                        ...
                        //By default make it as locked
                        //document.getElementById("cbox2").checked = true;
                       // document.getElementById("redactor_image_height").setAttribute("disabled", true);
                        $('.checkbox-image').click(function (e) {
                            console.log(count);
                            var className = document.getElementById("checkbox-image").className;
                            if (count == 1) {
                                alertifyReset();
                                alertify.confirm("Are you sure ? This can distort your image.", function (e) {

                                });
                            }
                           
                            //console.log(className);
                            //var checked = $('.checkbox-image').classList.contains('checked');
                            //if (className === "checkbox-image") {
                            //    document.getElementById("redactor_image_height").value = "auto";
                            //    document.getElementById("redactor_image_height").setAttribute("disabled", true);
                            //}
                            //else {
                            //    $image.addClass('unlocked');
                            //    document.getElementById("redactor_image_height").value = imageHeight;
                            //    document.getElementById("redactor_image_height").removeAttribute("disabled");
                            //}
                        });
                        ...
                        $(document).on('keydown keypress', "#redactor_image_width", function () {
                            typewatch(function () {
                                isDimensions = true;
                                var className = document.getElementById("checkbox-image").className;
                                if (className === "checkbox-image")
                                    document.getElementById("redactor_image_height").value = $("#redactor_image_width").val() === "" ? 0 : Math.round(parseInt($("#redactor_image_width").val(), 10) * 3 / 4);

                            }, 500);
                        });

                        $(document).on('keydown keypress', "#redactor_image_height", function () {
                            typewatch(function () {
                                 isDimensions = true;
                                var className = document.getElementById("checkbox-image").className;
                                if (className === "checkbox-image")
                                    document.getElementById("redactor_image_width").value = $("#redactor_image_height").val() === "" ? 0 : Math.round(parseInt($("#redactor_image_height").val(), 10) * 4 / 3);
                        ...
                        $('#url-mergefields').click(function () {
                            if (this.checked == true) {
                                ...
                                $('<div class="form-group medium"><label class="control-label">[|Fields|] </label><input id="mergefields" /></div><div class="form-group medium"><a class="btn btn-lg btn-primary" id="btn-appendToURL">Add Field</a></div>').insertBefore($('#url-mergefields').parent());
                                ...
                                $('<span class="validationMessage" id="mergefield-validation" style="display:none">Please select a field</span>').insertAfter($('#redactor-modal-image-edit .k-widget.k-dropdown.k-header'))
                                $('#redactor-modal-image-edit').height(300);

                            }
                            $('#url-mergefields').parent().remove();

                        })
                        var openInNewTab = $('#redactor-image-link-blank')[0].checked;
                        $('#redactor-image-link-blank').parent().remove();
                        var redactorLinkBlank = '<label class="pullLabel-right" style="display: inline-block;float :right!important"><input type="checkbox" id="redactor-image-link-blank"> Open link in new tab</label>';
                        $(redactorLinkBlank).insertAfter($('#redactor-modal-image-edit label').first());
                        $('#redactor-image-link-blank')[0].checked = openInNewTab;
                        $('#redactor-modal label').first().css({ 'display': 'inline-block' });
                        $('#redactor-image-link-blank').parent().css({ 'display': 'inline-block' });
                        $('#redactor-image-link-blank').parent().addClass('pullLabel-right');
                    }
                },
                ...
                update: function ($image) {
                    ...
                    var specifiedWidth = $('#redactor_image_width').val();
                    var specifiedHeight = $('#redactor_image_height').val();
                    if (specifiedWidth != 'undefined' && specifiedWidth != null && !isNaN(specifiedWidth) &&  isDimensions) {
                        $image.removeAttr('width');
                        $image.removeAttr('max-width');
                        $image.attr('data-idp', true);
                        $image.css({ "width": specifiedWidth, "max-width": specifiedWidth });
                        $image.attr('width', specifiedWidth);
                    }
                    if ((specifiedHeight != 'undefined' && specifiedHeight != null && !isNaN(specifiedHeight) &&  isDimensions) || specifiedHeight == "auto") {
                        $image.removeAttr('height');
                        $image.attr('data-idp', true);
                        //console.log(specifiedHeight);
                        if (specifiedHeight == "auto") specifiedHeight + " !important";
                        else specifiedHeight + "px !important";
                        $image.css('height', specifiedHeight);
                    }
        ...
        modal: function () {
            return {
                ...
                closeHandler: function (e) {
                    ...
                    if (window.selfDesigner && selfDesigner.controlBeingAdded() == 'control-table' && $('#redactor-modal header').text() == "Insert Table") {
                        selfDesigner.removeCancelledTableWidget(true);
                        selfDesigner.cancelEditing();
                    }
                },
                close: function (e) {
                    if (e) {
                ...
                insertFooter: function (e) {
                    this.$modalFooter.append('<div class="footerin"><label><b>Note :</b> Upload image size should not exceed 250 kb (allowed formates: PNG, JPEG, GIF)</label></div>');
                }
            };
        },
        progress: function () {
        ...
        upload: function () {
            return {
                init: function (id, url, callback) {
                    ...
                    this.upload.$input = $('<input type="file" name="file" class="file-upload" />');
