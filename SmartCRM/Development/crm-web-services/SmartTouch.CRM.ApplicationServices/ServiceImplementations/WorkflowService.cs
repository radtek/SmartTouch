    ...
    public class WorkflowService : IWorkflowService
    {
        ...
        void hasAccess(short documentId, int? userId, int accountId, short roleId)
        {
            ...
            if (!isAccountAdmin)
            {
               ...
               // if (isPrivate && !workflowRepository.IsCreatedBy(documentId, userId.Value, accountId))
        ...
        public NotifyUserResponse NotifyUser(NotifyUserRequest request)
        {
            ...
            byte status = 0;
            status = accountRepository.GetAccountStatus(request.AccountId);
            if (status != (byte)AccountStatus.Suspend)
            {
                ...
                if (request != null && userPrimaryEmails != null)
                {
        ...
        {
            return workflowRepository.IsWorkflowHasTimerActions(workflowId);
        }
    }
}
