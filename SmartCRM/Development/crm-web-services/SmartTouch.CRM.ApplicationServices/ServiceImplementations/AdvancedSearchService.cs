    ...
    public class AdvancedSearchService : IAdvancedSearchService
    {
        ...
        public QuickSearchResponse QuickSearch(QuickSearchRequest request)
        {
            ...
            else
            {
                ...
                {
                    types.Add(typeof(Campaign));
                }
                if (userModules.Contains(16))
                {
                    types.Add(typeof(Opportunity));
                }
            ...
            bool isCampaignsPrivate = cachingService.IsModulePrivate(AppModules.Campaigns, request.AccountId);
            bool isFormsPrivate = cachingService.IsModulePrivate(AppModules.Forms, request.AccountId);
            bool isOpportunitiesPrivate = cachingService.IsModulePrivate(AppModules.Opportunity, request.AccountId);

            IList<AppModules> privateModules = new List<AppModules>();

            if (isContactsPrivate)
                privateModules.Add(AppModules.Contacts);
            if (isCampaignsPrivate)
                privateModules.Add(AppModules.Campaigns);
            if (isOpportunitiesPrivate)
                privateModules.Add(AppModules.Opportunity);
            if (isFormsPrivate)
        ...
        IEnumerable<Type> getSearchableTypes(IEnumerable<SearchableEntity> searchableEntities)
        {
            ...
            foreach (SearchableEntity entity in searchableEntities)
            {
                ...
                else if (entity == SearchableEntity.Companies)
                {
                    ...
                    types.Add(typeof(Tag));
                else if (entity == SearchableEntity.Campaigns)
                    types.Add(typeof(Campaign));
                else if (entity == SearchableEntity.Opportunities)
                    types.Add(typeof(Opportunity));
            }
            return types;
        }
        ...
        public async Task<List<int>> GetSavedSearchContactIds(GetSavedSearchContactIdsRequest request)
        {
            ...
            if (request.SearchDefinitionId != 0)
            {
                ...
                {
                    var splitstring = request.SearchCriteria.Split('$');
                    ...
                    if ((string.IsNullOrEmpty(sort) || sort.Equals("0")) && string.IsNullOrEmpty(name) && (!type.Equals("4")))
                     ...
                     types = type == "" ? new List<Type>() { typeof(Person) } : type == "1" ?
                ...
                if (isPrivate && !isAccountAdmin)
                {
                    ...
                    parameters.IsPrivateSearch = false;
                var SearchResult = await searchService.AdvancedSearchAsync(name, searchDefinition, parameters);
