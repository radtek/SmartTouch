...
namespace SmartTouch.CRM.ApplicationServices.ServiceImplementations
{
    public class ContactService : IContactService
    {
        ...
        public InsertPersonResponse InsertPerson(InsertPersonRequest request)
        {
            ...
            if (contact.CompanyID.HasValue && contact.CompanyID > 0)
            {
                ...
                if (Person.CompanyID == null)
                {
                    ...
                    //added by kiran on 23/05/2018 NEXG-3014
                    else if (request.RequestedFrom == RequestOrigin.Forms || request.RequestedFrom == RequestOrigin.API)
                        matchedQueriesResponse = this.FindMatchedSearchQuery(new FindMatchedSavedSearchQueryRequest() { Contact = newCompanyContact, AccountId = request.AccountId });
                    //end
        ...
        public UpdateContactCustomFieldResponse UpdateContactCustomField(UpdateContactCustomFieldRequest request)
        {
            ...
            {
                DateTime date = new DateTime();
                if (request.inputType == (short)FieldType.time)
                    DateTime.TryParse(request.Value, out date);
                else
                    DateTime.TryParseExact(request.Value, request.DateFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out date);
                Logger.Current.Informational("User value : " + request.Value);
                Logger.Current.Informational("Parsed value : " + date.ToString());
                Logger.Current.Informational("Format of user : " + request.DateFormat);
        ...
        public GetUntouchedContactsResponse GetUntouchedContactsIds(GetUntouchedContactsRequest request)
        {
            GetUntouchedContactsResponse response = new GetUntouchedContactsResponse();
            int userId = 0;
            bool isAccountAdmin = cachingService.IsAccountAdmin(request.RoleId, request.AccountId);
            bool isPrivate = cachingService.IsModulePrivate(AppModules.Contacts, request.AccountId);
            if (isPrivate && !isAccountAdmin)
            ...
            response.ContactIds = contactRepository.GetUntouchedContactsIds(request.AccountId, userId, request.StartDate, request.EndDate);
            return response;
        }

        public BulkoperationConactsInsertionResponse InsertBulkOperationConacts(BulkoperationConactsInsertionRequest request)
        {
            BulkoperationConactsInsertionResponse response = new BulkoperationConactsInsertionResponse();
            contactRepository.InsertBulkConactsData(request.ConactsData);
            return response;
        }
