...
namespace SmartTouch.CRM.ApplicationServices.ServiceImplementations
{
    public class AccountService : IAccountService
    {
        ...
        public GetBulkOperationDataResponse GetQueuedBulkOperationData(GetBulkOperationDataRequest getBulkOperationDataRequest)
        {
            GetBulkOperationDataResponse response = new GetBulkOperationDataResponse();
            response.BulkOperations = accountRepository.GetQueuedBulkoperations();
            if (response.BulkOperations != null)
            {
                response.BulkContactIDs = accountRepository.GetBulkContacts(response.BulkOperations.BulkOperationID);
            }
                ...
                if (dailySummaryEmails.Count > 0)
                {
                    foreach (var summaryEmail in dailySummaryEmails)
                    {
                        try
                        {
                            ...
                            status = accountRepository.GetAccountStatus(summaryEmail.AccountID);
        ...
        {
            accountRepository.UpdateIndexStatusToFail(request.ReferenceIds, request.Status);
            ...
            try
            {
                ...
                if (data != null)
                {
                    ...
                    for (int i = 0; i < columns.Count; i++)
                    {
                        ...
                        importDataViewModel.PreviewData = rows[0].ItemArray[i].GetType() == typeof(DateTime) && rows[0].ItemArray[i].ToString().Contains("12/31/1899") ? rows[0].ItemArray[i].ToString().Replace("12/31/1899", "") : rows[0].ItemArray[i].ToString();
            ...
            else
            {
                ...
                try
                {
                    ...
                    jobid = importRepository.InsertLeadAdapterjob(jobLog, uniqueidentifier, UpdateOnDuplicate, true,
                                                      AccountID, request.ImportDataListViewModel.UserId, duplicateLogic, (int)request.RequestedBy,request.ImportDataListViewModel.IncludeInReports,request.ImportDataListViewModel.LeadSourceId);
