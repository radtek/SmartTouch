        ...
        protected override void Configure()
        {
            ...
            this.CreateMap<Contact, ContactGridEntry>()
               ...
               .AfterMap((s, d) =>
               {
                   if (s.GetType().Equals(typeof(Person)))
                   {
                       ...
                       d.FirstLeadSourceId = s.LeadSources.Where(p => p.IsPrimary == true).Select(v => v.Id).FirstOrDefault();
                       d.LeadSourceIds = s.LeadSources.Select(v => v.Id).ToArray();
           ...
           .ForMember(c => c.Month, opt => opt.MapFrom(c => c.Month));
            ...
            this.CreateMap<BaseWorkflowAction, BaseWorkflowActionViewModel>()
                ...
                //.Include<WorkflowFollowUpAction, WorkflowFollowUpActionViewModel>();

            this.CreateMap<WorkflowAction, WorkflowActionViewModel>();

            //this.CreateMap<WorkflowFollowUpAction, WorkflowFollowUpActionViewModel>()
            //    .AfterMap((s, d) => 
            //    {
            //        if (!string.IsNullOrEmpty(s.ReminderType))
            //            d.SelectedReminderTypes = s.ReminderType.Split(',').Select(se => (ReminderType)int.Parse(se));
            //    });
