...
namespace SmartTouch.CRM.Repository.Repositories
{
    ...
    public class ImportDataRepository : Repository<ImportData, int, GetImportDataDb>, IImportDataRepository
    {
        ...
        /// <returns></returns>
        public int InsertLeadAdapterjob(LeadAdapterJobLogs leadAdapterJobLog, Guid uniqueIdentifier, bool updateOnDuplicate, bool isFromImport, int AccountID, int UserID, int DuplicteLogicID,int ownerId,bool IncludeInReports,short leadsourceId)
        {
            ...
            try
            {
                ...
                {   
                    ...
                    new SqlParameter {ParameterName="@LeadSourceId", Value=leadsourceId },
                   outputParam
                };
        ...
        /// <summary>
        /// For Geeting LeadSource Id By jobid and LeadAdapterType
        /// </summary>
        /// <param name="jobId"></param>
        /// <param name="typeId"></param>
        /// <returns></returns>
        public short GetLeadSourceIdByAccountIdAndLeadAdapterType(int accountId, byte typeId)
        {
            using(var db = ObjectContextFactory.Create())
            {
                var sql = @"SELECT AM.LeadSourceType FROM dbo.LeadAdapterAndAccountMap(NOLOCK) AM
                            INNER JOIN dbo.DropdownValues(NOLOCK) DDL ON DDL.AccountID = AM.AccountID AND DropdownID=5 AND IsActive = 1 
                            WHERE AM.AccountID=@AccountId AND ddl.IsDeleted = 0 AND AM.LeadAdapterTypeID=@LeadAdapterTypeId AND AM.IsDelete=0";
                return db.Get<short>(sql, new { AccountId = accountId, LeadAdapterTypeId = typeId }).FirstOrDefault();
            }
        }
    }
}
