...
namespace SmartTouch.CRM.Repository.Repositories
{
    public class TourRepository : Repository<Tour, int, TourDb>, ITourRepository
    {
        ...
        public int GetLifeCycleStageForTour(int[] contactIds, int accountId)
        {
            var db = ObjectContextFactory.Create();

            var sql = @"select lifecyclestage from contacts where contactid in (SELECT DataValue FROM dbo.Split(@contactIds, ',')) and isdeleted = 0 and accountid = @accountId and lifecyclestage is not null
                        group by lifecyclestage";
            IEnumerable<int> count = db.Get<int>(sql, new { contactIds = string.Join(",", contactIds.ToArray()), accountId = accountId });
            return (count != null && count.Any()) ? count.Count() == 1 ? count.FirstOrDefault() : 0 : 0;
        }

        public void UpdateLifeCycleStageForTourContacts(int[] contactIds, short lifeCycleStage, int userId)
        {
            var db = ObjectContextFactory.Create();

            IEnumerable<ContactsDb> Contacts = db.Contacts.Where(w => contactIds.Contains(w.ContactID));
            if (Contacts != null && Contacts.Any())
            {
                foreach (var con in Contacts)
                {
                    con.LifecycleStage = lifeCycleStage;
                    con.LastUpdatedOn = DateTime.UtcNow;
                    con.LastUpdatedBy = userId;
                    db.Entry<ContactsDb>(con).State = System.Data.Entity.EntityState.Modified;

                    //IndexData indexData = new IndexData();
                    //indexData.CreatedOn = DateTime.UtcNow;
                    //indexData.IndexType = 1;
                    //indexData.EntityID = con.ContactID;
                    //indexData.Status = 1;
                    //indexData.ReferenceID = Guid.NewGuid();
                    //indexData.IsPercolationNeeded = true;
                    //db.Entry<IndexData>(indexData).State = System.Data.Entity.EntityState.Added;
                }
                db.SaveChanges();
            }
        }
