...
namespace SmartTouch.CRM.Repository.Repositories
{
    public class AccountRepository : Repository<Account, int, AccountsDb>, IAccountRepository
    {
            ...
            using (var db = ObjectContextFactory.Create())
            {
                if (accountId != 0)
                    account = db.Accounts.Where(a => a.AccountID == accountId).Select(s => new Account()
                    {
                        ...
                        Id = s.AccountID,
                        DomainURL = s.DomainURL,
                        Email = new Email() { EmailId = s.PrimaryEmail }
                    }).FirstOrDefault();
            }
            return account;
        }
        ...
        /// <summary>
        /// Getting Queued Bulkoperation.
        /// </summary>
        /// <returns></returns>
        public BulkOperations GetQueuedBulkoperations()
        {
            using(var db = ObjectContextFactory.Create())
            {
                BulkOperationsDb bulkOperationsDb = db.BulkOperations.Where(b => b.Status == (byte)BulkOperationStatus.InQueue).FirstOrDefault();
                if (bulkOperationsDb != null)
                    return Mapper.Map<BulkOperationsDb, BulkOperations>(bulkOperationsDb);
                else
                    return null;
        ...
        /// <param name="accountId"></param>
        /// <returns></returns>
        public byte GetAccountStatus(int accountId)
        {
            var db = ObjectContextFactory.Create();
            var sql = @"SELECT Status FROM Accounts (NOLOCK) WHERE AccountID = @id";
            byte status = db.Get<byte>(sql, new { id = accountId }).FirstOrDefault();
            return status;
        }
