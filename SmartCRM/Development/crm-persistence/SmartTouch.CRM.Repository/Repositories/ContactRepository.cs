...
namespace SmartTouch.CRM.Repository.Repositories
{
    public class ContactRepository : Repository<Contact, int, ContactsDb>, IContactRepository
    {
        ...
        public IEnumerable<Contact> FindAll(IList<int> ContactIDs, bool isElastic = true)
        {
          ...
          //  IEnumerable<ContactLastNote> contactLastNote = null;
            ...
            db.QueryStoredProc(sp, (reader) =>
                 {
                     ...
                     if (isElastic)
                     {
                        // contactLastNote = reader.Read<ContactLastNote>().ToList();
        ...
        public IEnumerable<Owner> GetUsers(int AccountId, int UserId, bool IsSTadmin)
        {
            ...
            if (UserId == 0 && IsSTadmin)
                sql = @"SELECT UserId as OwnerID, FirstName + ' ' + LastName +' (' + PrimaryEmail + ') ' AS OwnerName, IsDeleted, AccountID,Status  FROM Users (NOLOCK) WHERE AccountId IN (@accountId, 1) AND Status = @status";
            else if (UserId == 0 && !IsSTadmin)
                sql = @"SELECT UserId as OwnerID, FirstName + ' ' + LastName +' (' + PrimaryEmail + ') ' AS OwnerName, IsDeleted, AccountID,Status  FROM Users (NOLOCK) WHERE AccountId IN (@accountId) AND Status = @status";
            else
                sql = @"SELECT UserId as OwnerID, FirstName + ' ' + LastName +' (' + PrimaryEmail + ') ' AS OwnerName, IsDeleted, AccountID,Status  FROM Users (NOLOCK) WHERE AccountId IN (@accountId, 1) AND Status = @status and UserId = @userId";
        ...
        {
            Logger.Current.Informational("Contact entered to update Contacts_Audit contactId:" + contactID);
            ...
            Logger.Current.Informational("Contact update to Contacts_Audit successfully contactId:" + contactID);
        ...
        /// <returns></returns>
        public IDictionary<int,int> GetContactByPhoneNumberID(IEnumerable<int> ContactPhoneNumberIDs)
        {
            var db = ObjectContextFactory.Create();
            var sql = @"SELECT PE.ContactPhoneNumberID,PE.ContactID FROM ContactPhoneNumbers(NOLOCK) PE
                        INNER JOIN @tbl T ON T.ContactId = PE.ContactPhoneNumberID
                        WHERE PE.IsDeleted = 0";
            return db.Get<phoneNumberObject>(sql, new { tbl = ContactPhoneNumberIDs.AsTableValuedParameter("dbo.Contact_List") }).Select(s => new { s.ContactPhoneNumberID, s.ContactID } ).ToDictionary( d => d.ContactPhoneNumberID, v => v.ContactID);
        ...
        public IEnumerable<Person> GetContactsToSync(int accountId, int userId, int? maxNumRecords, DateTime? timeStamp, bool firstSync, CRUDOperationType operationType)
        {
            ...
            if (firstSync)
            {
                ...
                var query = @"SELECT ContactID FROM Contacts (NOLOCK)
                            WHERE OwnerID = @UserId AND IsDeleted = 0 AND ContactType = 1
                            AND AccountID = @AccountId AND ContactID NOT IN (SELECT ContactID FROM @tbl)";
                var firstTimeSyncContacts = db.Get<int>(query, new { UserId = userId, AccountId = accountId, tbl = contactIds.AsTableValuedParameter("dbo.Contact_List") });
            ...
            //var contacts1ToSyncDb = db.Contacts
            ...
            var contactIdsToSync = db.CRMOutlookSync.Where(c => c.SyncStatus == (short)OutlookSyncStatus.NotInSync && c.EntityType == (byte)AppModules.Contacts)
                .Select(c => c.EntityID);
            var contactsSP = this.FindAll(contactIdsToSync.ToList(), true);
            if (contactsSP.IsAny())
        ...
        public void InsertBulkOperation(BulkOperations bulkOperations, int[] contactIds)
        {
            ...
            bulkOperationsdb.Status = (byte)BulkOperationStatus.InQueue;
            using (var db = ObjectContextFactory.Create())
            {
        ...
        public IEnumerable<int> GetUntouchedContactsIds(int accountId, int userId, DateTime startDate, DateTime endDate)
        {
            using(var db = ObjectContextFactory.Create())
            {
                var sql = @"SELECT ContactID FROM Contacts (NOLOCK) C
                            WHERE C.AccountID=@AccountId AND C.IsDeleted=0 AND C.IncludeInReports=1 AND COALESCE(C.OwnerID,0)=COALESCE(NULLIF(@UserId,0),C.OwnerID,0) AND ContactType=1
                            AND C.LastContactedThrough IS NULL AND C.LastUpdatedOn BETWEEN @StartDate AND @EndDate";
                return db.Get<int>(sql, new { AccountId = accountId, UserId = userId, StartDate = startDate, EndDate = endDate }).ToList();
            }
        }

        /// <summary>
        /// For Inserting Bulkoperation Conacts
        /// </summary>
        /// <param name="conactsData"></param>
        public void InsertBulkConactsData(IEnumerable<BulkContact> conactsData)
        {
            using (var db = ObjectContextFactory.Create())
            {
                List<BulkContactData> TempDb = (Mapper.Map<IEnumerable<BulkContact>, IEnumerable<BulkContactData>>(conactsData)).ToList();
                db.BulkInsert<BulkContactData>(TempDb);
                db.SaveChanges();
            }
        }
    }
