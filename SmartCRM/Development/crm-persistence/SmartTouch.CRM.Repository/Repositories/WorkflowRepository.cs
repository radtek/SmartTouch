    ...
    public class WorkflowRepository : Repository<Workflow, short, WorkflowsDb>, IWorkflowRepository
    {
        ...
        private ICollection<WorkflowTriggersDb> GetTriggersByWorkflowId(short workflowID)
        {
            ...
            var triggers = db.WorkflowTriggers.Include(x => x.Forms).Include(x => x.Campaigns)
                                      ...
                                      .Include(x => x.TourTypeDropdownValue)
        ...
        public IEnumerable<SearchDefinition> GetAllSmartSearches(int AccountID)
        {
            ...
            var sql = @"SELECT SD.SearchDefinitionID,SD.AccountID,SD.SearchDefinitionName FROM SearchDefinitions (NOLOCK) SD
                        WHERE SD.SelectAllSearch=0 AND SD.AccountID=@AccountId AND SD.IsDeleted=0
        ...
        /// <summary>
        /// Checking the workflow has time actions or not.
        /// </summary>
        /// <param name="workflowId"></param>
        /// <returns></returns>
        public bool IsWorkflowHasTimerActions(int workflowId)
        {
            using(var db = ObjectContextFactory.Create())
            {
                var sql = @"DECLARE @HasTimerActions bit=1
                            IF NOT EXISTS(SELECT TOP 1 * FROM WorkflowActions (NOLOCK) WA WHERE WA.WorkflowID=@WorkflowId AND WA.WorkflowActionTypeID=3)
                            BEGIN
                                SET	@HasTimerActions=0
                            END
                            SELECT @HasTimerActions";
                return db.Get<bool>(sql, new { WorkflowId = workflowId }).FirstOrDefault();
            }
        }
