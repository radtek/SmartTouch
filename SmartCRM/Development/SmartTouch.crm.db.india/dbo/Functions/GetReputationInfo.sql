...
BEGIN	
	...
	SET @SenderRepCount = (SELECT CASE WHEN COUNT(1) > 1000 THEN 20 ELSE 0 END 
		...
		SELECT CR.DeliveryStatus, CR.HasComplained, COUNT(CR.CampaignRecipientID)
			FROM dbo.vCampaignRecipients CR WITH (NOLOCK)
	...
	IF (@AllRecpCount > 0)
		BEGIN
			...
			-- OPEN RATE
			SET @OpenRate = (SELECT COUNT(1) FROM dbo.vCampaignStatistics CS WITH (NOLOCK)
									INNER JOIN dbo.vCampaignRecipients CR (NOLOCK) ON CS.CampaignRecipientID = CR.CampaignRecipientID AND CS.CampaignID = CR.CampaignID AND CS.AccountID = CR.AccountID
