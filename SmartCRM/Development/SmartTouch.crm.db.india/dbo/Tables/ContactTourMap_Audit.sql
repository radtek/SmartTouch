CREATE TABLE [dbo].[ContactTourMap_Audit] (
    ...
    CONSTRAINT [PK_ContactTourMap_Audit] PRIMARY KEY CLUSTERED ([AuditId] ASC) WITH (FILLFACTOR = 90) ON [SECONDARY]
);
...
CREATE NONCLUSTERED INDEX [IX_ContactTourMap_Audit_ContactID_AuditAction]
    ON [dbo].[ContactTourMap_Audit]([ContactID] ASC, [AuditAction] ASC) WITH (FILLFACTOR = 90)
    ON [SECONDARY];
...
CREATE NONCLUSTERED INDEX [IX_ContactTourMap_Audit_TourID]
    ON [dbo].[ContactTourMap_Audit]([TourID] ASC)
    INCLUDE([AuditId], [ContactTourMapID], [ContactID], [AuditAction], [AuditDate], [AuditUser], [AuditApp], [AuditStatus], [IsCompleted], [LastUpdatedBy], [LastUpdatedOn]) WITH (FILLFACTOR = 90)
    ON [SECONDARY];
