...
inner JOIN dbo.vCampaignRecipients cr  on cr.ContactID = c.ContactID AND cr.AccountID = C.AccountID
inner join dbo.vCampaignStatistics cs  on cs.CampaignRecipientID = cr.CampaignRecipientID AND CS.AccountID = C.AccountID --and cs.ActivityDate >  Convert(dateTime,DateAdd(dd,-120,GETUTCDATE()) ,120) 
