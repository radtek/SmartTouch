...
begin transaction
	...
	SELECT @Count = COUNT(1) FROM vCampaignRecipients (NOLOCK) WHERE CampaignID =@CampaignId AND AccountID = @AccountID
...
IF(@Count > 0)
	BEGIN
		...
		SELECT COUNT(1) FROM vCampaignRecipients (nolock) WHERE CampaignId = @CampaignId AND AccountID = @AccountID AND (DeliveryStatus = 116 OR DeliveryStatus = 113 OR DeliveryStatus is NULL)
	END
	ELSE
		BEGIN
			/*NewContacts*/
			IF @ResentStatus = 122
				BEGIN
					...
					SELECT CR.ContactID, CR.[To] FROM vCampaignRecipients CR (NOLOCK)
			...
			ELSE IF @ResentStatus = 123
				BEGIN
					...
					SELECT CR.ContactID, CR.[To]  FROM vCampaignStatistics CS (NOLOCK)
					INNER JOIN ResentCampaigns RC (NOLOCK) ON ((CS.CampaignID = RC.ParentCampaignID) OR (CS.CampaignID = RC.CampaignID))
					INNER JOIN vCampaignRecipients CR (NOLOCK) ON CS.CampaignRecipientID = CR.CampaignRecipientID
			...
			;WITH ActiveContacts AS
				(
					...
					SELECT CR.ContactID FROM vCampaignStatistics CS (NOLOCK)
					INNER JOIN vCampaignRecipients CR (NOLOCK) ON CR.CampaignRecipientID = CS.CampaignRecipientID AND Cr.AccountID = CS.AccountID
				...
				;WITH FinalRecipients AS
				 ...
				 SELECT ROW_NUMBER() OVER ( ORDER BY ContactId), CONTACTID, EMAIL FROM FinalRecipients 

				 DELETE FROM @FinalRecipients WHERE dbo.isValidEmailFormat_New(Email) = 0
				...
				BEGIN
					...
					INSERT INTO vCampaignRecipients --WITH (TABLOCK) 
					([CampaignID]
				   ,[ContactID]
				   ,[CreatedDate]
				   ,[To]
				...
				SELECT COUNT(1) FROM vCampaignRecipients (NOLOCK) WHERE CampaignID = @CampaignId AND AccountID = @AccountID
		END
 COMMIT TRANSACTION
END TRY
BEGIN CATCH    
