...
BEGIN
	...
	SELECT F.FormSubmissionID as Id, F.ContactID, F.IPAddress,DDV.DropdownValue,F.SubmittedOn, F.SubmittedData, COUNT(1) OVER() AS Total 
	...
	WHERE F.FormID = @FormID AND F.SubmittedOn BETWEEN COALESCE( @StartDate,CAST('1753-1-1' as datetime)) AND COALESCE(@EndDate,GETUTCDATE()) AND C.IsDeleted = 0
