...
BEGIN
	...
	SET @SQL =	@SQL + N'
					from	dbo.vCampaignRecipients cr  WITH(NOLOCK) inner join dbo.Contacts c  WITH(NOLOCK) on c.ContactID = cr.ContactID AND C.AccountID = CR.AccountID
	...
	END
	SET @SQL =	@SQL + N'
					WHERE	CampaignID = @CampaignID AND (cr.DeliveryStatus IS NULL OR cr.DeliveryStatus = 116) AND (@IsLinkedToWorkflow = 0 OR (@IsLinkedToWorkflow = 1 AND (cr.WorkflowID > 0)))
				)tmp '

	PRINT @SQL

	EXEC sp_executesql @SQL, N'@FieldList CustomFieldList READONLY, @CampaignID int, @AccountID int, @IsLinkedToWorkflow bit', @FieldList, @CampaignID, @AccountID, @IsLinkedToWorkflow
...
/*
	...
	FROM	(
				select	c.ContactID ContactID2, cr.CampaignRecipientID CRID, c.FirstName, c.LastName, c.Company, c.Title, ce.Email EmailID,
						...
						a.AddressLine1 ADDLINE1, a.AddressLine2 ADDLINE2, a.City, s.StateName [State], a.ZipCode, cn.CountryName Country,
						tmp.*
				from	dbo.vCampaignRecipients cr inner join dbo.Contacts c on c.ContactID = cr.ContactID
