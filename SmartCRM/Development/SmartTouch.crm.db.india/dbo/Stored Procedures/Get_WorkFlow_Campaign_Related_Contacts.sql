...
BEGIN
	...
	IF (@CampaignDrillDownType = 6) -- Sent = 6
		BEGIN
			INSERT INTO	@Contacts SELECT DISTINCT  ContactID  FROM vCampaignRecipients (NOLOCK) 
    ...
    IF(@CampaignDrillDownType = 1 OR @CampaignDrillDownType = 2 OR @CampaignDrillDownType = 4 OR @CampaignDrillDownType = 5)--Opened = 1, Clicked = 2 , Complained = 4 , unsubscribed = 5
		BEGIN
			INSERT INTO @Contacts SELECT DISTINCT  VR.ContactID  FROM vCampaignStatistics (NOLOCK) VS
			INNER JOIN vCampaignRecipients (NOLOCK) VR ON VR.CampaignRecipientID = VS.CampaignRecipientID AND VR.AccountID = @AccountId
