...
BEGIN
	...
	BEGIN TRY
		...
		SELECT TOP 1 @LeadAdapterJobLogID = LeadAdapterJobLogID, @ContactSource = 2, @SourceType = 11, @OwnerID = CreatedBy, @fileName = [FileName]
			FROM dbo.LeadAdapterJobLogs(NOLOCK) WHERE LeadAdapterJobStatusID = 7 ORDER BY LeadAdapterJobLogID ASC --Read To Process
	...
	BEGIN
		...
		SELECT @DupUpdate = UpdateOnDuplicate, @DuplicateLogic = DuplicateLogic, @TrackMessages = 2 FROM dbo.ImportDataSettings (NOLOCK) WHERE LeadAdaperJobID = @LeadAdapterJobLogID
		...
		SELECT @AddressTypeID = DDL.DropdownValueID, @AccountID = AM.AccountID, @LeadAdapterAndAccountMapID = AM.LeadAdapterAndAccountMapID,
			...
			WHERE JL.LeadAdapterJobLogID = @LeadAdapterJobLogID
		...
		IF (ISNULL(@AddressTypeID, 0) = 0)
			BEGIN
				SET @AddressTypeID = (SELECT TOP 1 DropdownValueID FROM dbo.DropdownValues WHERE DropdownID = 2 AND IsActive = 1 AND AccountID = @AccountID)	
			END
		...
		LEFT JOIN dbo.DropdownValues LCSDefault  (NOLOCK) ON LCSDefault.AccountID = ICD.AccountID AND LCSDefault.DropdownID = 3 AND LCSDefault.IsActive = 1 AND LCSDefault.IsDefault = 1
		...
		ELSE IF (ISNULL(@DuplicateLogic, 2) = 2)
			...
			--			AND ICD.ValidEmail = 1 AND ICD.IsDuplicate=1
				...
				UPDATE ImportContactData
					...
					WHERE C.IsDeleted = 0 AND ICD.JobID = @leadAdapterJobLogID AND ICD.PrimaryEmail = ''
					AND (ICD.IsBuilderNumberPass = 1 OR ICD.IsBuilderNumberPass IS NULL)
					 AND (ICD.IsCommunityNumberPass = 1 OR ICD.IsCommunityNumberPass IS NULL)
			...
			as
			(
				select ContactId, Max(ImportContactDataID) as ImportContactDataID  from dbo.ImportContactData (NOLOCK)  where JobId = @leadAdapterJobLogID and ContactId is not null
				AND IsDuplicate=0
		...
		IF ((ISNULL(@DupUpdate, 1) = 1 AND ISNULL(@DuplicateLogic, 2) = 1))
			BEGIN
				EXEC [dbo].[Process_Import_Contacts]  @leadAdapterJobLogID, @accountID, @ownerId, @ContactSource, @SourceType, 1;
			END				
		ELSE IF ((ISNULL(@DupUpdate, 1) = 1 AND ISNULL(@DuplicateLogic, 2) = 2))
			BEGIN
				EXEC [dbo].[Process_Import_Contacts]  @leadAdapterJobLogID, @accountID, @ownerId, @ContactSource, @SourceType, 2;		
			END 
		ELSE IF ((ISNULL(@DupUpdate, 1) = 0 AND ISNULL(@DuplicateLogic, 2) = 1))
			BEGIN
				EXEC [dbo].[Process_Import_Contacts]  @leadAdapterJobLogID, @accountID, @ownerId, @ContactSource, @SourceType, 3;
			END
		ELSE IF ((ISNULL(@DupUpdate, 1) = 0 AND ISNULL(@DuplicateLogic, 2) = 2))
			BEGIN
				EXEC [dbo].[Process_Import_Contacts]  @leadAdapterJobLogID, @accountID, @ownerId, @ContactSource, @SourceType, 4;
			END
		...
		INTO #tEamilStatus FROM dbo.ImportContactData(NOLOCK) ICD
			LEFT JOIN dbo.ImportContactsEmailStatuses(NOLOCK) ICES ON ICD.ReferenceID = ICES.ReferenceID
       ...
       select ContactID,FirstName,LastName,Company,CommunicationID,Title,ContactImageUrl,AccountID,LeadScore,LeadSource,HomePhone,WorkPhone,MobilePhone,PrimaryEmail,ContactType,SSN,LifecycleStage
		...
		,LastContactedThrough,IsLifecycleStageChanged,NewLifecycleStage,FirstContactSource,FirstSourceType, 1 as RowNumber into #tContactAudit from dbo.Contacts_Audit where 1 <> 1;
              ...
              WHERE C.IsDeleted = 0 AND CL.JobID = @leadAdapterJobLogID
					 ...
					 and c.ContactID NOT IN (SELECT Contactid from Contacts_Audit WHERE Accountid = @accountid and contacttype = 2 );
		...
		,LogDate
		,AccountID
		,EntityName, 1 as RowNumber into #tUserActivityLogs from dbo.UserActivityLogs where 1 <> 1;
		...
		IF @SourceType = 1
			...
			select TOP 1 @SubmissionFieldID = FieldID from Fields (NOLOCK)  where accountid = @AccountID and IsLeadAdapterField = 1 and LeadAdapterType = 1 and Title like '%Lead Submission ID%'
			select TOP 1 @FileNameFieldID = FieldID from Fields (NOLOCK)  where accountid = @AccountID and IsLeadAdapterField = 1 and LeadAdapterType = 1 and Title like '%File Name%'
		...
		FROM dbo.ImportContactData(NOLOCK) ICD
		INNER JOIN DropDownValues D ON D.DropDownValue = ICD.LeadSource and D.AccountID = ICD.AccountID
				WHERE JobID = @leadAdapterJobLogID AND ContactID > 0 AND ValidEmail = 1 and ContactStatusID <> 0 AND IsDuplicate=0
				and dropdownid = 5 
		...
		BEGIN
			...
			SELECT @leadAdapterJobLogID, @NSubject, @NSubject, GETUTCDATE(), 1 ,@OwnerID,23, NULL
		END
		ELSE
		BEGIN
			INSERT INTO Notifications
			SELECT @leadAdapterJobLogID, @NSubject, @NSubject, GETUTCDATE(), 1 ,@OwnerID,19, NULL
		END		
		  ...
		  declare @dupIdList table
			...
			while exists(select 1 from @dupIdList)
				begin
					...
					insert into LeadAdapterJobLogs(LeadAdapterAndAccountMapID,StartDate,EndDate,LeadAdapterJobStatusID,Remarks,FileName,CreatedBy,CreatedDateTime,StorageName)
					select LeadAdapterAndAccountMapID,StartDate,EndDate,1,Remarks,FileName,CreatedBy,CreatedDateTime,StorageName from LeadAdapterJobLogs (NOLOCK) 
