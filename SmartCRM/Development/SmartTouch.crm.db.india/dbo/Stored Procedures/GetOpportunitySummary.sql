...
BEGIN
	...
	SELECT O.* INTO #OppTemp FROM OpportunityContactMap(NOLOCK)	OCM
	INNER JOIN Opportunities(NOLOCK) O ON O.OpportunityID = OCM.OpportunityID
	WHERE OCM.ContactID = @ContactID AND O.CreatedOn > @Period
	ORDER BY O.ExpectedClosingDate, O.CreatedBy DESC
