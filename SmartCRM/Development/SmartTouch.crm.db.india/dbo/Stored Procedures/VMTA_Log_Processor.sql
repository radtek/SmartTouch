...
BEGIN
	...
	SELECT TOP 1000 CR.ContactID,CLD.CampaignLogDetailsID,CR.CampaignId,CR.CampaignRecipientID,CLD.Recipient,CLD.DeliveryStatus,CLD.OptOutStatus,CLD.BounceCategory,CLD.TimeLogged,CLD.Remarks,CLD.Status,CLD.FileType,CLD.CreatedOn,ROW_NUMBER() OVER(ORDER BY CampaignLogDetailsID) AS RowId 
	into #CampaignDetails
	FROM CampaignLogDetails CLD WITH (NOLOCK)
	LEFT JOIN vCampaignRecipients CR WITH (NOLOCK) ON (CLD.CampaignId = CR.CampaignID OR CLD.CampaignRecipientId = CR.CampaignRecipientId) AND CR.[To] = CLD.Recipient AND CR.WorkflowID IS NULL	
	WHERE CLD.Status = 1
	...
	SELECT GETUTCDATE()
		...
		BEGIN
			 ...
			 @CampaignRecipientId = CampaignRecipientId,@DetailID = CampaignLogDetailsID FROM #CampaignDetails WHERE RowId = @LoopCounter	
			...
			UPDATE ContactEmails SET EmailStatus = (CASE WHEN @Optout_Status IS NULL THEN @Email_Status 
														ELSE @Optout_Status 
													END )
					WHERE ContactID = @ContactID AND Email = @Recipient AND IsDeleted = 0 		 
		...
		FROM vCampaignRecipients CR  WITH (NOLOCK)
		...
		UPDATE CampaignLogDetails SET Status = 2  WHERE CampaignLogDetailsID IN (SELECT CampaignLogDetailsID FROM #CampaignDetails  WITH (NOLOCK) WHERE CampaignId IS NULL)
