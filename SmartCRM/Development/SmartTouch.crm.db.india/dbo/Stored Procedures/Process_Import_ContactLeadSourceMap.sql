...
BEGIN
		...
		select ContactID, LeadSourceID, cast(1 as bit) as IsPrimaryLeadSource, 0 as RowNumber into #tLeadSourceMap from dbo.ImportContactData (NOLOCK)  where 1 <> 1;

		;WITH tempData
		as
		(
			SELECT ContactID, LeadSourceID, case when len(LeadSource) > 0 then 0 else 1 end as  IsPrimaryLeadSource  FROM dbo.ImportContactData(NOLOCK) WHERE JobID = @leadAdapterJobLogID AND ContactID > 0 
			...
			UNION
			select ICD.ContactID, DV.DropdownValueID as LeadSourceID , case when   EXISTS (SELECT IsPrimaryLeadSource FROM ContactLeadSourceMap (NOLOCK)  WHERE ContactID IN (SELECT ContactID FROM ImportContactData WHERE  JobID = @leadAdapterJobLogID) AND IsPrimaryLeadSource = 1 )  then 0 else 1 end as IsPrimaryLeadSource from ImportContactData(NOLOCK) ICD
	    ...
	    select td.ContactID, LeadSourceID, td.IsPrimaryLeadSource, ROW_NUMBER() OVER(ORDER BY td.ContactID, LeadSourceID) from tempData td
