...
BEGIN
	...
	IF @IsDefaultDateRange > 0
		BEGIN
			...
			;WITH CurrrentMonthClicks AS (
				SELECT E.CampaignID, COUNT(1) CurrentMonth FROM vCampaignStatistics (NOLOCK) CS
			...
			select CampaignID,count(1) CurrentMonthContacts from (
				SELECT DISTINCT E.CampaignID, VCR.ContactID FROM vCampaignStatistics (NOLOCK) CS
				INNER JOIN vCampaignRecipients	VCR ON VCR.CampaignRecipientID = CS.CampaignRecipientID AND VCR.AccountID = @AccountID
				INNER JOIN @EntitiesTable E ON E.LinkID = CS.CampaignLinkID
				WHERE CS.ActivityType = 2 AND CS.ActivityDate BETWEEN @CurrentMonthFrom AND @CurrentMonthTo AND CS.AccountID = @AccountID) tmp
				GROUP BY CampaignID)
			, LastMonthClicks AS (		
				SELECT  E.CampaignID, COUNT(1) LastMonth FROM vCampaignStatistics (NOLOCK) CS
				INNER JOIN @EntitiesTable E ON E.LinkID = CS.CampaignLinkID
			...
			, Last12MonthsClicks AS (
				SELECT  E.CampaignID, COUNT(1) Last12Months FROM vCampaignStatistics (NOLOCK) CS
				INNER JOIN @EntitiesTable E ON E.LinkID = CS.CampaignLinkID
	...
	ELSE
		BEGIN
			...
			print @ToDate
			   ;WITH TotalClicks AS (
				SELECT  E.CampaignID, COUNT(1) TotalClicks FROM vCampaignStatistics (NOLOCK) CS
					INNER JOIN @EntitiesTable E ON E.LinkID = CS.CampaignLinkID
