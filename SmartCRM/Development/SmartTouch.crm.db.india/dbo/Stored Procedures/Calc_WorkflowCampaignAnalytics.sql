...
BEGIN
	...
	SELECT CR.CampaignID,CR.ContactID, CR.CampaignRecipientID,CR.DeliveryStatus, CR.HasComplained, CR.OptOutStatus, CR.WorkflowID FROM  
	vCampaignRecipients CR (NOLOCK) WHERE  WorkflowID = @WorkflowID AND CR.AccountID = @AccountID AND CampaignID = COALESCE(@CampaignID,CampaignID) AND CR.DeliveredOn BETWEEN @FromDate AND @ToDate
	...
	WHILE @CampaignID > 0
		BEGIN
			...
			SELECT @Opened = COUNT(DISTINCT CS.CampaignRecipientID) FROM vCampaignStatistics (nolock) CS
			...
			SELECT @Clicked = COUNT(DISTINCT CampaignRecipientID) FROM vCampaignStatistics (nolock) CS 
            INNER JOIN @CampaignAnalytics CA ON CA.RecipientID = CS.CampaignRecipientID
			...
			INSERT INTO @WorkflowCampaignAnalytics(CampaignID,  Delivered, Bounced ,Opened, Clicked, Complained, OptedOut)
	...
	SELECT C.Name AS CampaignName, WA.* FROM @WorkflowCampaignAnalytics WA
