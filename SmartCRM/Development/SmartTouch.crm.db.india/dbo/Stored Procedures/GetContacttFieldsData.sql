...
CREATE PROCEDURE [dbo].[GetContacttFieldsData]
	@ContactId INT,
	@Fields VARCHAR(MAX),
...
--from fields where (accountid is null or accountid = 94) AND StatusID = 201 AND FieldCode != '' AND AccountID IS NULL
	...
	INNER JOIN dbo.Split_2(@Fields,',') S ON S.DataValue = F.FieldID
	UNION
	...
	WHILE @Counter > 0
		BEGIN
			...
			ELSE IF @FieldType = 'C' OR @FieldType = 'D'
				BEGIN
					...
					WHERE F.FieldType = 'C'
					UNION
					select fl.FieldID, fl.FieldName, dv.PhoneWithFormat from GET_DropdownFieldValues (nolock) dv 
					inner join DropdownValues dvs (nolock) on dvs.DropdownValueID = dv.CustomFieldID and contactid = @ContactId 
					RIGHT join @FieldsList fl on fl.FieldID = dvs.DropdownValueID
					where  fl.FieldType = 'D'
		...
		IF @NotificationType = 3 -- Forms
			BEGIN 
				SELECT SubmittedData FROM FormSubmissions (NOLOCK) WHERE FormSubmissionID=@EntityID
			END
...
--EXEC [dbo].[GetContacttFieldsData] 1741755,'1,2,7,3,24,25,22,26,19882,19727,19726,19890,19734,19736,4652,4653',12,0,4218
--1740634
...
--select * from ContactPhoneNumbers where contactid = 1740719
