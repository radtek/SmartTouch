...
BEGIN
	...
	BEGIN TRY
			...
			BEGIN
				DELETE	CR
				FROM	dbo.vCampaignRecipients AS CR  INNER JOIN(
							SELECT TOP (@RecordPerBatch) CampaignRecipientID
							FROM dbo.vCampaignRecipients AS CR (NOLOCK)
				...
				SET @TotalRecordsDeleted = @TotalRecordsDeleted + @RecordsDeleted
			END
			PRINT CAST(@TotalRecordsDeleted AS VARCHAR) + ' records deleted from vCampaignRecipients'
			...
			BEGIN
				DELETE	CS
				FROM	dbo.vCampaignStatistics AS CS  INNER JOIN(
							SELECT TOP (@RecordPerBatch) CampaignTrackerID
							FROM dbo.vCampaignStatistics AS CS (NOLOCK)
				...
				SET @TotalRecordsDeleted = @TotalRecordsDeleted + @RecordsDeleted
			END
			PRINT CAST(@TotalRecordsDeleted AS VARCHAR) + ' records deleted from vCampaignStatistics'
			...
			BEGIN
				DELETE	CRS
				FROM	dbo.vCampaignRecipients AS CRS  INNER JOIN(
							SELECT TOP (@RecordPerBatch) CampaignRecipientID
							FROM dbo.vCampaignRecipients AS CRS (NOLOCK)
