...
namespace SmartTouch.CRM.Repository.Repositories
{
    public class CampaignRepository : Repository<Campaign, int, CampaignsDb>, ICampaignRepository
    {
        ...
        public Campaign GetNextCampaignToTrigger()
        {
            ...
            var campaignDb = default(CampaignsDb);
            using (var db = ObjectContextFactory.Create())
            {
                try
                {
                    ...
                    if (campaignDb != null)
                    {
                        ...
                        if (campaignDb.IncludePlainText)
                        {
        ...
        public void ReScheduleCampaign(int campaignId, DateTime scheduleDate)
        {
            if (campaignId != 0)
            {
                ...
                string sql = @"UPDATE Campaigns
                           ...
                           WHERE CampaignID = @CampaignID";
                db.Execute(sql, new { CampaignID = campaignId, ScheduleTime = scheduleDate });
            }
        }
        ...
        public void ReScheduleAutomationCampaign(int campaignId, DateTime scheduleDate)
        {
            if (campaignId != 0)
            {
                ReScheduleCampaign(campaignId, scheduleDate);

                var db = ObjectContextFactory.Create();
                string sql = "UPDATE CampaignRecipients " +
                             "SET ScheduleTime = @ScheduleTime " +
                             "WHERE CampaignID = @CampaignID AND DeliveryStatus IS NULL AND WorkflowID IS NOT NULL";
                db.Execute(sql, new { CampaignID = campaignId, ScheduleTime = scheduleDate });
            }
        }
        ...
        public IEnumerable<CampaignRecipient> GetNextAutomationCampaignRecipients()
        {
            ...
            var campaignReipients = db.Get<CampaignRecipientsDb>(sql).ToList();
            return Mapper.Map<List<CampaignRecipientsDb>, List<CampaignRecipient>>(campaignReipients);
        }
        ...
        //NEXG-3005, NEXG-3004 and NEXG-3007
