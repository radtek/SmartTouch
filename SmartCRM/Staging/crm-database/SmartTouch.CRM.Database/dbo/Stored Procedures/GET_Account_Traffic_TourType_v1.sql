...
CREATE procedure [dbo].[GET_Account_Traffic_TourType_v1](
	@AccountID			int,
	@Type				tinyint,
	@StartDate          datetime,
	@EndDate            datetime,
	@SelectedIDs		VARCHAR(MAX),
	@TrafficType		VARCHAR(MAX),
	@ContactListID		int = 0
)
AS
BEGIN
	...
	VALUES('GET_Account_Traffic_TourType_v1', @AccountID)
	...
	FROM	(
				...
				WHERE	DropdownID = 8
						...
						AND DropdownValueID IN (SELECT DataValue FROM dbo.Split(@TrafficType, ','))
			) tmp
		...
		BEGIN
			SET @SQL = '
					...
					FROM	DropdownValues(NOLOCK) dv INNER JOIN Tours t ON t.CommunityID = dv.DropdownValueID
							...
							INNER JOIN users(NOLOCK) u on u.accountid IN (dv.accountid, 1) and u.UserID = UTM.UserID
							INNER JOIN contacts(NOLOCK) c on c.AccountID = dv.AccountID and c.contactid = ctm.ContactID
							INNER JOIN DBO.DropdownValues(NOLOCK) DV2 ON U.AccountID = DV2.AccountID AND t.tourtype = dv2.DropdownValueID
...
exec GET_Account_Traffic_TourType_v1
		@AccountID = 45,
		@Type = 0,
