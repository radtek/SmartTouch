...
Create table #Table (ID  INT IDENTITY(1,1),AccountID int ,AccountName varchar(100),ObjectName VARCHAR (100), ContactID INT,DuplicatecCount INT,DuplicatecCountSTATUS TINYINT)
 ...
 ;
  ...
  AS (SELECT
    ...
    ContactEmailID,
    ce.ContactID,
    ROW_NUMBER() OVER (PARTITION BY AccountName,CE.AccountID, EMAIL, ce.ContactID ORDER BY ContactEmailID ASC) AS DuplicatecCount
  FROM ContactEmails ce WITH (NOLOCK)
  inner join Accounts a ON A.Accountid =ce.Accountid
  INNER JOIN  Contacts  C ON C.ContactID  = CE.ContactID
  WHERE CE.IsDeleted = 0 and ce.IsPrimary = 1 AND C.Isdeleted =  0
  )
...
--  --/ For duplicate ContactLeadSourceMap /
