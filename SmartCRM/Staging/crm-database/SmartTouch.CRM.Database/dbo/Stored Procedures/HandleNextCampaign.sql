...
begin transaction
	...
	set @IsAdmin= (SELECT  CASE WHEN  @OwnerId IN (SELECT  USERID  FROM  Users  u 
     inner join  roles  r on r.roleid = u.roleid 
     where (ROLENAME = 'Super Administrator' OR ROLENAME ='Account Administrator')  AND U.userid = @OwnerId ) THEN 1 ELSE 0  END USERID )
...
IF(@Count > 0)
	...
	ELSE
		BEGIN
			...
			drop table  if exists #FinalContacts
			...
			;WITH ActiveContacts AS
				(
					...
					AND C.AccountID = @AccountID AND CA.LastUpdatedOn > @ActivityDuration  AND (C.FirstContactSource !=2 OR  C.FirstContactSource IS NULL)
				)
				, TagContacts AS
				(	
					--get active contacts
				...
				Select * INTO #FinalContacts from (SELECT * FROM TagContacts  UNION SELECT * FROM SDContacts)T

				IF(@IsAdmin=0)
				BEGIN
					INSERT INTO @Recipients (ContactId, Email)  
					SELECT CCTM.ContactID, CE.Email FROM
					(SELECT * FROM #FinalContacts) CCTM
