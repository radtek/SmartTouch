...
BEGIN
	...
	WHERE C.AccountID = @AccountID AND C.IsDeleted = 0 AND CCM.IsDeleted = 0 AND C.IncludeInReports=1) x
	UNION
	...
	BEGIN
		...
		WHERE C.AccountID = @AccountID AND C.IsDeleted = 0 AND C.IncludeInReports=1
	...
	BEGIN
		...
		INNER JOIN ContactLeadSourceMap CLM (nolock) on clm.ContactID = c.ContactID and c.IsDeleted = 0 AND C.IncludeInReports=1 and c.AccountID = @AccountID and CLM.LastUpdatedDate BETWEEN @StartDate AND @EndDate
		AND clm.IsPrimaryLeadSource = 1 
	...
	BEGIN
		...
		FROM Contacts (NOLOCK) C
		INNER JOIN ContactLeadSourceMap CLM (NOLOCK) ON CLM.ContactID = C.ContactID AND C.AccountID = @AccountID AND C.IsDeleted = 0 AND C.IncludeInReports=1 AND CLM.LastUpdatedDate BETWEEN @StartDate AND @EndDate
		AND CLM.IsPrimaryLeadSource = 1
	...
	BEGIN
		...
		SELECT  ddv.DropdownValueID, CONVERT(varchar(10), ET.id), COUNT(CLM.ContactID) 
		FROM Contacts (NOLOCK) C
		INNER JOIN ContactLeadSourceMap CLM (NOLOCK) ON CLM.ContactID = C.ContactID AND C.AccountID = @AccountID AND C.IsDeleted = 0 AND C.IncludeInReports=1 AND CLM.LastUpdatedDate 
