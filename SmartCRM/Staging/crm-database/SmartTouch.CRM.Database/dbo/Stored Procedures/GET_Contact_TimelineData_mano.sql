CREATE PROCEDURE [dbo].[GET_Contact_TimelineData_mano]  
 (  
  @AccountID  int,  
  @ContactID  int  
 )  
...
BEGIN  
 ...
 SET NOCOUNT ON  
 BEGIN TRY   
  DECLARE @DEFADMIN INT  
  SELECT @DEFADMIN = value FROM EnvironmentSettings (NOLOCK) WHERE Name = 'CRM User'  
  
  SELECT ROW_NUMBER() OVER (ORDER BY ContactID) TimelineID, ContactID, Module, AuditAction,   
   Value, AuditDate, ModuleID, UserName, CreatedBy, AuditStatus, IsAPIForm   
  FROM (   
  ...
  SELECT ContactID, Module, CASE WHEN AuditAction = 'I' THEN 'created'  
         ...
         END AuditAction,   
    Value, AuditDate, ModuleID,CASE WHEN UserName <> '' THEN UserName ELSE (U.FirstName+ ' '+ U.LastName) END UserName , TIMELINE.CreatedBy, AuditStatus, IsAPIForm    
  FROM   
  (  
   SELECT CA.ContactID, 'Contact' Module,   
    ...
    CA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm  
   ...
   SELECT CA.ContactID, 'Lifecycle' Module, 'U' AuditAction,  
    ...
    CA.AuditDate, CA.ContactID ModuleID, '' UserName, CA.LastUpdatedBy CreatedBy, CA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm     
    FROM dbo.Contacts_Audit(NOLOCK) CA   
   ...
   SELECT CNMA.ContactID, 'Note' Module, NA.AuditAction, convert(nvarchar(max),NA.NoteDetails +'|'+Convert(varchar(1),na.AddToContactSummary)+'|') Value, NA.AuditDate,  
       CNMA.NoteID ModuleID, '' UserName, NA.CreatedBy, CNMA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm      
   ...
   SELECT  CTMA.ContactID, 'Action' Module,  
      AA.AuditAction, ISNULL(AA.ActionDetails,'') Value, AA.AuditDate,  
            AA.ActionID ModuleID, '' UserName, AA.LastUpdatedBy, CTMA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm    
      ...
      ISNULL(TAA.ActionDetails,'') Value, CAMA.AuditDate, CAMA.ActionID ModuleID, '' UserName, CAMA.LastUpdatedBy, CAMA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm      
   ...
   SELECT CRMA.ContactID, 'Relationship' Module, CRMA.AuditAction,  
    ...
    ELSE ISNULL(C.Company, '') +', '+ DV.DropdownValue END Value, CRMA.AuditDate, CRMA.ContactRelationshipMapID ModuleId,  
     '' UserName, CRMA.CreatedBy CreatedBy, CRMA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm        
  ...
  SELECT  CTMA.ContactID, 'Tour' Module, TR.AuditAction, ISNULL(TR.TourDetails,'') Value, TR.AuditDate,  
   TR.TourID ModuleID, '' UserName, TR.LastUpdatedBy, CTMA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm     
  ...
  SELECT  CTMA.ContactID, 'Tour' Module,   
   ...
   ISNULL(TAA.TourDetails,'') Value, CTMA.AuditDate, CTMA.TourID ModuleID, '' UserName, CTMA.LastUpdatedBy, CTMA.AuditStatus, CAST(0 AS BIT) AS IsAPIForm          
  ...
  SELECT CR.ContactID, 'Campaign' Module, 'S' AuditAction, C.[Name] Value, CR.SentOn AuditDate,  
   CR.CampaignID ModuleID, C.SenderName UserName,'' createdby, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm  
   ...
   WHERE CR.ContactID = @ContactID AND C.AccountID = @AccountID AND CR.DeliveryStatus NOT IN (116,110,113)  
  
  UNION ALL  
  
  SELECT FS.ContactID, 'Form' Module, 'SU' AuditAction, F.[Name] Value, FS.SubmittedOn AuditDate,  
   FS.FormSubmissionID ModuleID, '' UserName, CASE WHEN F.LastModifiedBy IS NULL THEN    
  CASE WHEN EXISTS(SELECT * FROM Users (NOLOCK) WHERE UserID = F.CreatedBy AND AccountID = 1) THEN @DEFADMIN  
  ELSE F.CreatedBy END ELSE CASE WHEN EXISTS(SELECT * FROM Users (NOLOCK) WHERE UserID = F.LastModifiedBy AND AccountID = 1) THEN @DEFADMIN  
  ELSE F.LastModifiedBy END END, CONVERT(BIT, 1) AuditStatus, F.IsAPIForm  
  ...
  --SELECT FS.ContactID, 'Form' Module, 'SU' AuditAction, F.[Name] Value, FS.SubmittedOn AuditDate,  
  -- FS.FormSubmissionID ModuleID, '' UserName, @DEFADMIN, CONVERT(BIT, 1) AuditStatus  
  -- FROM dbo.FormSubmissions(NOLOCK) FS   
  --  INNER JOIN dbo.Forms(NOLOCK) F ON FS.FormID = F.FormID  
  -- WHERE FS.ContactID = @ContactID AND F.AccountID = @AccountID  
  ...
  SELECT D.ContactID, 'Attachment' Module, 'I' AuditAction, (D.OriginalFileName + '~' + D.FilePath) Value,  
   D.CreatedDate AuditDate, CONVERT(INT,  D.DocumentID) ModuleID, '' UserName, D.CreatedBy, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm  
  ...
  SELECT C.ContactID, 'Email' Module, 'S' AuditAction, SMD.[Subject] Value, CEA.SentOn AuditDate,  
   SMD.SentMailDetailID ModuleID, '' UserName, CEA.SentBy, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm  
  ...
  UNION ALL  
   ...
   rmi.ReceivedMailID ModuleID,  c.FirstName + ' '+ c.LastName UserName, rma.UserId, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm  
  ...
  SELECT C.ContactID, 'Text' Module, 'S' AuditAction, TRD.[Message] Value, CEA.SentOn AuditDate,   
   CEA.ContactPhoneNumberID ModuleID, '' UserName, CEA.SentBy, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm  
  ...
  SELECT C.ContactID, 'Lead Adapter' Module, 'SU' AuditAction, LT.Name Value, LAJD.CreatedDateTime AuditDate,  
   LAJD.LeadAdapterJobLogDetailID ModuleID, '' UserName, NULL, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm  
  ...
  SELECT CTM.ContactID, 'Tag' Module, 'I' AuditAction, TA.[TagName] Value, CTM.TaggedOn AuditDate,  
      CTM.TagID ModuleID, '' UserName, TA.CreatedBy, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm      
  ...
  SELECT CWV.ContactID, 'Web Visit' Module, 'WV' AuditAction, CWV.PageVisited Value,  
    ...
    (ISNULL(C.FirstName, '') +' '+ISNULL(C.LastName, '')) UserName, '' CreatedBy, CONVERT(BIT, 1) AuditStatus, CAST(0 AS BIT) AS IsAPIForm   
