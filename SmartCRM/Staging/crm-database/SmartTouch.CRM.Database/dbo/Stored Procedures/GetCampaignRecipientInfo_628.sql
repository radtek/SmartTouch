CREATE PROCEDURE [dbo].[GetCampaignRecipientInfo_628](
	@CampaignID int,  
...
BEGIN
	...
	DECLARE @SQL NVARCHAR(MAX) = N'SELECT  CONTACTID, @CFList 
	FROM ( SELECT   ContactId, FieldCode ,FieldValue FROM #contactFields) AS S  
	PIVOT 
	( 
		MAX(FieldValue) 
