...
BEGIN
	...
	BEGIN
		IF(@ColIndex = 2)
		BEGIN 
			SELECT C.ContactID as contactID, '' as firstname, '' as lastname, '' as email, '' as phonenumber,0 as ContactEmailID FROM Contacts (NOLOCK) C
			...
			AND CLM.isprimaryleadsource = 1 AND C.IncludeInReports=1
		END
		ELSE 
		BEGIN
			SELECT C.ContactID as contactID, '' as firstname, '' as lastname, '' as email, '' as phonenumber,0 as ContactEmailID FROM Contacts (NOLOCK) C
			INNER JOIN ContactLeadSourceMap (NOLOCK) CLM ON CLM.ContactID = C.ContactID AND CLM.LastUpdatedDate BETWEEN @StartDate AND @EndDate
			WHERE C.AccountId = @AccountID AND C.IsDeleted = 0 AND CLM.LeadSouceID = @LeadSourceID AND C.IncludeInReports=1
