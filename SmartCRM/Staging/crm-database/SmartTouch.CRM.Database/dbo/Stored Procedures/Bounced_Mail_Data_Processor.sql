...
SET @LastRunDate = DateADD(mi, -1,  GETUTCDATE());
	...
	;WITH Recipientdata
	As(
		 SELECT DISTINCT  ContactID,[To] FROM CampaignRecipients (nolock) WHERE LastModifiedOn >= @LastRunDate
	...
	AS (
		...
		INNER JOIN CampaignRecipients (nolock) cr ON rd.ContactID = cr.ContactID AND rd.[To] = cr.[To]
