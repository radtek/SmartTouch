...
BEGIN
	...
	BEGIN
			...
			WHERE C.AccountID = @AccountID AND C.IsDeleted = 0 AND C.IncludeInReports=1 AND CLM.LastUpdatedDate BETWEEN @StartDate AND @EndDate
	...
	BEGIN
			...
			( 
				...
				WHERE CLM.IsPrimaryLeadSource = 1 AND C.IncludeInReports=1
				GROUP BY ET.ID, dv.dropdownvalue
			)
