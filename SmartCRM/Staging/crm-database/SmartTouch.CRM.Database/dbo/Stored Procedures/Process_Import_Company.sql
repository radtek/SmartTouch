...
BEGIN
		...
		WHILE (1 = 1)
		BEGIN
			insert into Contacts (Company,ReferenceID, AccountID, ContactType, IsDeleted, LastUpdatedOn,FirstContactSource,FirstSourceType, OwnerID, ContactSource,LastUpdatedBy,IncludeInReports)
			SELECT CompanyName, NEWID(), ICD.AccountID, 2, 0, GETUTCDATE(),@ContactSource, @sourceType, @ownerId,@ContactSource,@ownerId,0
		...
		(
			...
			join dbo.ImportContactData ICD (nolock) ON C.Company = ICD.CompanyName AND C.AccountID = ICD.AccountID and ICD.JobID = @LeadAdapterJobLogID	 --AND C.ContactID = ICD.CompanyID
		...
		AND   ReferenceID IN (SELECT ReferenceID FROM Contacts WITH  (NOLOCK) WHERE ContactType = 2 and AccountID = @AccountID and  IsDeleted = 0
		))
