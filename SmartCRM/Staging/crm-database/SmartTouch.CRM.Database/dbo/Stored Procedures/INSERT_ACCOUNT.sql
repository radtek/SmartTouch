...
BEGIN 
	...
	SET NOCOUNT ON
		...
		BEGIN TRY			
				...
				SELECT @SubscriptionID = SubscriptionID, @AccountName = AccountName, @UserID = CreatedBy, @HelpURL = HelpURL, @UserLimit = UserLimit, @ExcludedRoles = ExcludedRoles  FROM @AccountInfo			
				...
				BEGIN 
					...
					SELECT R1.RoleID, M.ModuleID
						FROM Roles(NOLOCK) R 
							INNER JOIN dbo.RoleModuleMap(NOLOCK) RMM ON RMM.RoleID = R.RoleID
							INNER JOIN dbo.Modules(NOLOCK) M ON M.ModuleID = RMM.ModuleID
							INNER JOIN dbo.Roles(NOLOCK) R1 ON R.RoleName = R1.RoleName AND R1.AccountID = @AccountID
				...
				BEGIN 
					...
					SELECT dv.DropdownID, @AccountID,dv.DropdownValue, dv.IsDefault, dv.SortID, dv.DropdownValueTypeID, dv.IsActive, 0
						...
						INNER JOIN dbo.SubscriptionDefaultDropdownValueMap(NOLOCK) sdv on dv.DropdownValueID = sdv.DropdownValueID and sdv.SubscriptionID = @SubscriptionID							
				...
				BEGIN
                  ...
                  INSERT INTO dbo.SubscriptionModuleMap(SubscriptionID, ModuleID, AccountID, Limit, ExcludedRoles)
				  SELECT SubscriptionID, ModuleID,@AccountID, @UserLimit, @ExcludedRoles
                       FROM dbo.SubscriptionModuleMap(NOLOCK)
				...
				BEGIN 
					...
					SELECT @AccountID,DropdownValueID,
						...
						END
					FROM dbo.DropdownValues(NOLOCK) WHERE AccountID = @AccountID AND DropdownID = 6
				...
				BEGIN 
					...
					SELECT @AccountID,ReportName,ReportType
						FROM dbo.Reports(NOLOCK) WHERE AccountID IS NULL AND ReportType != 13
				...
				BEGIN 
					...
					SELECT ServiceProviderID, CommunicationTypeID, LoginToken, ProviderName, EmailType, IsDefault, SenderPhoneNumber, ROW_NUMBER() OVER(ORDER BY ServiceProviderID) AS ROWNUMBER,ImageDomainID
						  FROM dbo.ServiceProviders(NOLOCK) WHERE AccountID = 1 AND CommunicationTypeID IN (1,2)
					...
					WHILE (@Loopcounter <= @Recordcount)
						BEGIN	
							...
							SELECT @ProviderEmail = Email from AccountEmails(NOLOCK) where ServiceProviderID = @OldServiceProviderID 
							...
							IF (@CommunicationTypeID = 1)
								BEGIN
									...
									SELECT @UniqueID, Name, Host, APIKey, UserName, [Password], Port, IsSSLEnabled, MailProviderID, VMTA, SenderDomain
											FROM  EnterpriseCommunication.dbo.MailRegistration(NOLOCK) WHERE [Guid] = (SELECT LoginToken FROM @ServiceProvidersInfo WHERE ROWNUMBER = @Loopcounter)
								END
							ELSE 
								BEGIN
									...
									SELECT @UniqueID, @TwilioFriendlyName, [Address], APIKey, Token, UserName, [Password], TextProviderID
											FROM  EnterpriseCommunication.dbo.TextRegistration(NOLOCK) WHERE [Guid] = (SELECT LoginToken FROM @ServiceProvidersInfo WHERE ROWNUMBER = @Loopcounter)
								END
				...
				BEGIN 
					...
					SELECT s.SearchDefinitionID, s.SearchDefinitionName, s.ElasticQuery, s.SearchPredicateTypeID, s.CustomPredicateScript, s.IsFavoriteSearch, s.IsPreConfiguredSearch,sds.SearchDefinitionSubscriptionMapID MapID,
							    ...
							    FROM dbo.SearchDefinitions(NOLOCK) s
								INNER JOIN dbo.SearchDefinitionSubscriptionMap(NOLOCK) sds on s.SearchDefinitionID = sds.SearchDefinitionID and sds.SubscriptionID = @SubscriptionID									
					...
					WHILE (@Loopcounter <= @Recordcount)
						BEGIN
							...
							SET @SearchDefinitionID = SCOPE_IDENTITY()
							 ...
							 SET @DropdownValueid = (CASE WHEN (select count(*) from @SearchDefinitionInfo where MapID = 2) >= 1  THEN
							 ...
							 SET @HomePhoneId = (CASE WHEN (select count(*) from @SearchDefinitionInfo where MapID = 1) >= 1  THEN
							 ...
							 SET @MobilePhoneId = (CASE WHEN (select count(*) from @SearchDefinitionInfo where MapID = 1) >= 1  THEN
							 ...
							 SET @WorkPhoneId = (CASE WHEN (select count(*) from @SearchDefinitionInfo where MapID = 1) >= 1  THEN
							...
							SELECT (CASE WHEN (SF.FieldID = 5 or SF.FieldID = 4 or SF.FieldID = 6)  THEN  NULL ELSE SF.FieldID END) ,
									...
									FROM dbo.SearchFilters(NOLOCK) SF
