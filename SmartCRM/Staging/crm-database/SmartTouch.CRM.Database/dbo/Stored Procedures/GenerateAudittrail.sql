...
BEGIN
	...
	BEGIN
		...
		SET @CreateStatement = @CreateStatement + '[AuditStatus] [bit] NOT NULL)' 
	...
	EXEC ('CREATE TRIGGER tr_' + @TableName + '_Insert ON ' + @Owner + '.' + @TableName + ' FOR INSERT AS INSERT INTO ' + @TableName + @AuditNameExtention + '(' +  @ListOfFields + 'AuditAction, AuditStatus) SELECT ' + @ListOfFields + '''I'', 1 FROM Inserted')

	EXEC ('CREATE TRIGGER tr_' + @TableName + '_Update ON ' + @Owner + '.' + @TableName + ' FOR UPDATE AS INSERT INTO ' + @TableName + @AuditNameExtention + '(' +  @ListOfFields + 'AuditAction, AuditStatus) SELECT ' + @ListOfFields + '''U'', 1 FROM Inserted')

	EXEC ('CREATE TRIGGER tr_' + @TableName + '_Delete ON ' + @Owner + '.' + @TableName + ' FOR DELETE AS INSERT INTO ' + @TableName + @AuditNameExtention + '(' +  @ListOfFields + 'AuditAction, AuditStatus) SELECT ' + @ListOfFields + '''D'', 0 FROM Deleted')
