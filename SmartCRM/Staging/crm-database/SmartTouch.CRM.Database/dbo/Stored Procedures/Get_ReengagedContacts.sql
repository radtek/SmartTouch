...
BEGIN
	...
	IF @IsDefaultDateRange = 1
		BEGIN
			...
			BEGIN
        		SELECT distinct c.ContactID from CampaignStatistics(NOLOCK) cs
					...
					inner join CampaignRecipients(NOLOCK) cr on cs.CampaignRecipientID = cr.CampaignRecipientID AND cr.AccountID = @AccountId
	...
	ELSE
		BEGIN
			IF @HasSelectedLinks = 0
				BEGIN
        			SELECT distinct cr.ContactID from CampaignStatistics(NOLOCK) cs
						inner join CampaignLinks(NOLOCK) cl on cl.CampaignLinkID = cs.CampaignLinkID
						inner join WorkflowCampaignActionLinks(NOLOCK) wca on cl.CampaignLinkID = wca.LinkID
						inner join CampaignRecipients(NOLOCK) cr on cs.CampaignRecipientID = cr.CampaignRecipientID AND cr.AccountID = @AccountId
						inner join contacts(NOLOCK) c on cr.ContactID = c.ContactID
