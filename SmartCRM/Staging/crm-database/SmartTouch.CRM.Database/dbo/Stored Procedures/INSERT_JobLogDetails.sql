...
BEGIN
	...
	BEGIN TRANSACTION
		...
		IF (CONVERT(smallint, @isfromimport) = 1)
			BEGIN
				INSERT INTO dbo.ImportDataSettings (UpdateOnDuplicate, UniqueImportIdentifier, AccountID, ProcessBy, ProcessDate, DuplicateLogic, LeadAdaperJobID,IncludeInReports,LeadSourceID)
				VALUES (@updateonduplicate, @importuniqueidentifier, @AccountID, @UserID, GETUTCDATE(), @DuplicateLogic, @LeadAdapterID,@IncludeInReports,@LeadSourceId)
			END
