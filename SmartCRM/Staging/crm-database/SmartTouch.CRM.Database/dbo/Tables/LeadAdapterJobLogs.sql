CREATE TABLE [dbo].[LeadAdapterJobLogs] (
    ...
    CONSTRAINT [PK_LeadAdapterJobLogs] PRIMARY KEY CLUSTERED ([LeadAdapterJobLogID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON),
...
CREATE NONCLUSTERED INDEX [IX_LeadAdapterJobLogs_LeadAdapterAndAccountMapID]
    ...
    INCLUDE([LeadAdapterJobLogID], [StartDate], [EndDate], [LeadAdapterJobStatusID], [Remarks], [FileName], [CreatedBy], [CreatedDateTime], [StorageName]) WITH (FILLFACTOR = 90, PAD_INDEX = ON);
...
CREATE NONCLUSTERED INDEX [IX_LeadAdapterJobLogs_LeadAdapterJobStatusID]
    ...
    INCLUDE([LeadAdapterAndAccountMapID]) WITH (FILLFACTOR = 90, PAD_INDEX = ON);
...
CREATE NONCLUSTERED INDEX [IX_LeadAdapterJobLogs_LeadAdapterAndAccountMapID_LeadAdapterJobStatusID]
    ...
    INCLUDE([LeadAdapterJobLogID], [Remarks], [FileName], [CreatedDateTime]) WITH (FILLFACTOR = 90, PAD_INDEX = ON);
