CREATE TABLE [dbo].[Tours] (
    ...
    CONSTRAINT [PK_Tours] PRIMARY KEY CLUSTERED ([TourID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON),
    FOREIGN KEY ([LifeCycleStage]) REFERENCES [dbo].[DropdownValues] ([DropdownValueID]),
...
CREATE NONCLUSTERED INDEX [IX_Tours_AccountID]
    ...
    INCLUDE([TourID], [CommunityID], [CreatedBy], [CreatedOn]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];
...
CREATE NONCLUSTERED INDEX [IX_Tours_CommunityID]
    ...
    INCLUDE([TourID], [TourDate], [TourType]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];
...
CREATE NONCLUSTERED INDEX [IX_Tours_CreatedBy_NotificationStatus_RemindbyPopup_ReminderDate]
    ...
    INCLUDE([TourID], [TourDetails], [CommunityID], [TourDate], [TourType], [CreatedOn], [RemindbyText], [RemindbyEmail], [EmailRequestGuid], [TextRequestGuid], [AccountID], [LastUpdatedBy], [LastUpdatedOn]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];


GO
CREATE NONCLUSTERED INDEX [IX_Tours_missing_647]
    ON [dbo].[Tours]([AccountID] ASC)
    INCLUDE([TourID], [CommunityID], [TourDate], [CreatedBy]);


GO
CREATE NONCLUSTERED INDEX [NonClusteredIndex-20171109-125500]
    ON [dbo].[Tours]([TourDate] ASC, [AccountID] ASC, [CommunityID] ASC)
    INCLUDE([TourID]);
