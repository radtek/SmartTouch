CREATE TABLE [dbo].[CampaignRecipients_0004] (
    ...
    CONSTRAINT [PK_CampaignRecipients_0004] PRIMARY KEY CLUSTERED ([CampaignRecipientID] ASC, [CampaignID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON) ON [SecondaryFG],
    CONSTRAINT [CampaignRecipients_0004_CampaignID] CHECK ([CampaignID]>=(7501) AND [CampaignID]<=(10000)),
    CONSTRAINT [UNIQUE_CampaignRecipients_0004_CampaignRecipientID_CampaignID] UNIQUE NONCLUSTERED ([CampaignRecipientID] ASC, [CampaignID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON) ON [SecondaryFG]
);
...
CREATE NONCLUSTERED INDEX [IX_CampaignRecipients_CampaignID_HasUnsubscribed_0004]
    ON [dbo].[CampaignRecipients_0004]([CampaignID] ASC, [HasUnsubscribed] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SecondaryFG];
...
CREATE NONCLUSTERED INDEX [IX_CampaignRecipients_CampaignID_SentOn_CampaignRecipientID_0004]
    ...
    INCLUDE([CampaignRecipientID], [DeliveryStatus], [HasComplained]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SecondaryFG];
...
CREATE NONCLUSTERED INDEX [IX_CampaignRecipients_CampaignID_WorkflowID_HasComplained_0004]
    ON [dbo].[CampaignRecipients_0004]([CampaignID] ASC, [WorkflowID] ASC, [HasComplained] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SecondaryFG];


GO
CREATE NONCLUSTERED INDEX [IX_CampaignRecipients_ContactID_0004]
    ON [dbo].[CampaignRecipients_0004]([ContactID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SecondaryFG];
...
CREATE NONCLUSTERED INDEX [IX_CampaignRecipients_WorkflowID_0004]
    ...
    INCLUDE([DeliveryStatus], [HasUnsubscribed]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SecondaryFG];


GO
CREATE STATISTICS [_dta_stat_1746157316_9_1]
    ON [dbo].[CampaignRecipients_0004]([DeliveredOn], [CampaignRecipientID]);
