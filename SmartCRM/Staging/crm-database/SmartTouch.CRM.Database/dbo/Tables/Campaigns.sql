CREATE TABLE [dbo].[Campaigns] (
    ...
    [IncludePlainText]          BIT             DEFAULT ('0') NOT NULL,
    [HasDisclaimer]             BIT             CONSTRAINT [DF_Campaigns_HasDisclaimer] DEFAULT ((0)) NULL,
    CONSTRAINT [PK_Campaigns] PRIMARY KEY CLUSTERED ([CampaignID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON),
...
CREATE NONCLUSTERED INDEX [IX_Campaigns_CampaignStatusID_ScheduleTime]
    ...
    INCLUDE([CampaignID], [CampaignTemplateID], [Name], [Subject], [ServiceProviderID], [AccountID], [IsDeleted], [HTMLContent], [From], [CreatedBy], [CreatedDate]) WITH (FILLFACTOR = 90, PAD_INDEX = ON);


GO
CREATE NONCLUSTERED INDEX [IX_Campaigns_missing_19]
    ON [dbo].[Campaigns]([IsDeleted] ASC, [IsRecipientsProcessed] ASC, [CampaignStatusID] ASC, [ScheduleTime] ASC)
    INCLUDE([CampaignID], [CreatedDate]);
...
GO
CREATE NONCLUSTERED INDEX [IX_Campaigns_missing_21]
    ON [dbo].[Campaigns]([CampaignStatusID] ASC, [IsDeleted] ASC, [IsRecipientsProcessed] ASC, [ProcessedDate] ASC)
    INCLUDE([CampaignID]);


GO
CREATE STATISTICS [_dta_stat_659533433_1_13]
    ON [dbo].[Campaigns]([CampaignID], [IsDeleted]);
