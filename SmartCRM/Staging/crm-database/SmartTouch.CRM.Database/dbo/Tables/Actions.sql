CREATE TABLE [dbo].[Actions] (
    ...
    [SelectAll]          BIT              NULL,
    [MailBulkId]         INT              NULL,
    CONSTRAINT [PK_Actions] PRIMARY KEY CLUSTERED ([ActionID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON),
...
GO
CREATE NONCLUSTERED INDEX [IX_Actions_missing_207]
    ON [dbo].[Actions]([AccountID] ASC)
    INCLUDE([ActionID], [ActionDetails], [CreatedOn], [ActionType], [ActionDate], [ActionEndTime]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];


GO
CREATE NONCLUSTERED INDEX [IX_Actions_CreatedBy_NotificationStatus_RemindbyPopup_RemindOn]
    ON [dbo].[Actions]([CreatedBy] ASC, [NotificationStatus] ASC, [RemindbyPopup] ASC, [RemindOn] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];
