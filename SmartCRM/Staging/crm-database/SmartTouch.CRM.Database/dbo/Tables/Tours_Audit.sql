CREATE TABLE [dbo].[Tours_Audit] (
    ...
    CONSTRAINT [PK_Tours_Audit] PRIMARY KEY CLUSTERED ([AuditId] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
);
...
CREATE NONCLUSTERED INDEX [IX_Tours_Audit_TourID_AccountID]
    ...
    INCLUDE([TourDetails], [AuditAction], [AuditDate], [LastUpdatedBy]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];
