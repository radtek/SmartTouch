CREATE TABLE [dbo].[FormSubmissions] (
    ...
    CONSTRAINT [PK_FormSubmissions] PRIMARY KEY CLUSTERED ([FormSubmissionID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON),
...
CREATE NONCLUSTERED INDEX [IX_FormSubmissions_FormID_ContactID]
    ON [dbo].[FormSubmissions]([FormID] ASC)
    INCLUDE([FormSubmissionID], [ContactID], [IPAddress], [SubmittedOn], [StatusID], [SubmittedData], [LeadSourceID]) WITH (FILLFACTOR = 90, PAD_INDEX = ON);
