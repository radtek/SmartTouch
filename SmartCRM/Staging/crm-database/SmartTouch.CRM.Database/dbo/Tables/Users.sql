...
GO
CREATE NONCLUSTERED INDEX [NonClusteredIndex-20170125-144206]
    ON [dbo].[Users]([FirstName] ASC, [LastName] ASC)
    INCLUDE([UserID]);
...
GO
CREATE NONCLUSTERED INDEX [IX_Users_FirstName_LastName_UserID]
    ON [dbo].[Users]([FirstName] ASC, [LastName] ASC)
    INCLUDE([AccountID], [UserID]) WITH (FILLFACTOR = 90);
...
GO
DISABLE TRIGGER [dbo].[tr_Users_Update]
    ON [dbo].[Users];
