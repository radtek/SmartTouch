CREATE TABLE [dbo].[ContactActionMap] (
    ...
    CONSTRAINT [PK_ContactActionMap] PRIMARY KEY CLUSTERED ([ContactActionMapID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON),
...
CREATE NONCLUSTERED INDEX [IX_ContactActionMap_ActionID]
    ...
    INCLUDE([ContactID], [IsCompleted]) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];


GO
CREATE NONCLUSTERED INDEX [IX_ContactActionMap_ContactID]
    ON [dbo].[ContactActionMap]([ContactID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON);


GO
CREATE NONCLUSTERED INDEX [IX_ContactActionMap_ActionID_ActionID]
    ON [dbo].[ContactActionMap]([ActionID] ASC) WITH (FILLFACTOR = 90, PAD_INDEX = ON)
    ON [SECONDARY];
