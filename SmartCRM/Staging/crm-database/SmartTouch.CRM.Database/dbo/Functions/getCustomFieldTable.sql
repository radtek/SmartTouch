	...
	DECLARE @LoopValueCount2 int = (SELECT MAX(LoopID) FROM @DataCustomFields)
		...
		BEGIN
			...
			BEGIN
					...
					BEGIN
						SET @CFVID = ISNULL(CONVERT(VARCHAR(15),(SELECT CustomFieldValueOptionID FROM SmartCRM.dbo.CustomFieldValueOptions
					...
					ELSE IF (@FieldInputTypeID IN (1,12))
						BEGIN
							...
							SELECT DISTINCT RTRIM(LTRIM(DataValue)) FROM SmartCRM.dbo.Split(@CFValue, ',') WHERE LEN(DataValue) > 0
