	...
	*/
	...
	ELSE IF @instance = 3
		BEGIN
			...
			UNION SELECT 14 UNION SELECT 15 UNION SELECT 16 UNION SELECT 17 UNION SELECT 18 UNION SELECT 0 UNION SELECT 20
	...
	SELECT TOP (@noOfRecords) TokenGuid, SMQ.RequestGuid, [From], PriorityID, ScheduledTime, QueueTime, DisplayName,
		...
		LEFT JOIN SmartCRM.dbo.Accounts AC ON AC.AccountID = S.AccountID
...
declare @MailQueue dbo.MailQueue
	EXEC [dbo].[updateAndGetNextQueue] @MailQueue, 3, 4
