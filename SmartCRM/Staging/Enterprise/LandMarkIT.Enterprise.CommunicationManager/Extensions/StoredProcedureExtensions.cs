...
namespace LandmarkIT.Enterprise.CommunicationManager.Extensions
{
    public static class StoredProcedureExtensions
    {
            ...
            try
            {
                ...
                if (requests != null && responses != null)
                {
                    requests.ForEach(item =>
                    {
                        ...
                        dataTable.Rows.Add(1, item.From, item.To, 1, item.Message, (byte)response.StatusID, response.ServiceResponse, item.RequestGuid, item.TokenGuid, item.ScheduledTime);
                        dataTable.AcceptChanges();
                    });
                }
        ...
        {
            var mergeFields = new Dictionary<string, Dictionary<string, string>>();
            try
            {
                var parms = new List<SqlParameter>
                {
                ...
                var result = context.ExecuteStoredProcedure<SendMailMergeFieldsRequest>("dbo.GetSendMailMergefieldValues", parms).ToList();
                {
                    ...
                    distinctContacts.ForEach(x =>
                    {
                        var mf = result
                                .Where(r => r.Email == x)
                                .ToSafeDictionary(k => k.FieldCode, v => v.FieldValue);
                        if (!mergeFields.ContainsKey(x))
                        {
                            mergeFields.Add(x, mf);
                        }
                    });
                }
            }
