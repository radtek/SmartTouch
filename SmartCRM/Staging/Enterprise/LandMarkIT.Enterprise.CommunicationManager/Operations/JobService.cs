...
namespace LandmarkIT.Enterprise.CommunicationManager.Operations
{
    public class JobService : BaseService
    {
        ...
        {
            return unitOfWork.CronJobsRepository.GetAll().ToList();
        }

        public CronJobDb GetCronJobByType(CronJobType cronJobID)
        {
            return unitOfWork.CronJobsRepository.SingleOrDefault(x => x.CronJobID == cronJobID);
        }
        ...
        public void StopCronJob(CronJobType cronJobID, DateTime stopTime)
        {
            using(var _unitOfWork = new EfUnitOfWork())
            {
                ...
                var cronJob = _unitOfWork.CronJobsRepository.Single(cj => cj.CronJobID == cronJobID);
                var lastNotify = cronJob.LastNotifyDateTime ?? DateTime.UtcNow;
                var timeToRunInMinutes = (stopTime - lastNotify).TotalMinutes;
                cronJob.IsRunning = false;
                cronJob.EstimatedTimeInMin = (short)timeToRunInMinutes;
