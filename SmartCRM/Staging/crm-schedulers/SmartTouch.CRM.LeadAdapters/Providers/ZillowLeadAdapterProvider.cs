...
{
    public class ZillowLeadAdapterProvider : BaseLeadAdapterProvider
    {
        IMailGunService mailGunService;
        IContactService contactService;
        ISearchService<Contact> searchService;
        IImportDataRepository importDataRepository;
        public ZillowLeadAdapterProvider(int accountId, int leadAdapterAndAccountMapID, ILeadAdaptersRepository leadAdaptersRepository, IServiceProviderRepository serviceProviderRepository,
        ...
        public override ImportContactsData GetContacts(string fileName, IEnumerable<FieldViewModel> customFields, int jobId, IEnumerable<DropdownValueViewModel> DropdownFields)
        {
            ...
            try
            {
                ...
                customFields = customFields.Where(i => i.IsLeadAdapterField && i.LeadAdapterType == (byte)LeadAdapterTypes.Zillow && i.StatusId == FieldStatus.Active);
                ...
                foreach (var lead in leads)
                {
                    ...
                    try
                    {
                        ...
                        foreach (XElement element in elements)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    var customField = customFields.Where(i => i.Title.Replace(" ", string.Empty).ToLower() == (element.Name.LocalName.ToLower() + "(" + LeadAdapterTypes.Zillow.ToString().ToLower() + ")")).FirstOrDefault();
                                    if (customField != null)
                                    {
                                        ...
                                        if (customField.FieldInputTypeId == FieldType.date || customField.FieldInputTypeId == FieldType.datetime || customField.FieldInputTypeId == FieldType.time)
                                        {
                                            ...
                                            if (DateTime.TryParse(element.Value, out converteddate))
                                            {
                                        ...
                                        {
                                            double number;
                                            if (double.TryParse(element.Value, out number))
                                            {
                            ...
                            {
                                Logger.Current.Error("An exception occured in Genereating old new values element in Zillow : " + element.Name.LocalName, ex);
                                continue;
                            }
                        }
                    }
                    ...
                    {
                        Logger.Current.Error("An exception occured in Genereating old new values element in Zillow : ", ex);
                    }
