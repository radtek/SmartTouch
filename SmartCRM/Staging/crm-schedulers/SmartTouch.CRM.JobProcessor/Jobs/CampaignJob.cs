...
namespace SmartTouch.CRM.JobProcessor.Jobs
{
    public class CampaignJob : BaseJob
    {
        ...
        protected override void ExecuteInternal(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                while (campaign != null)
                {
                    ...
                    Log.Informational(string.Format("Campaign {0} has been picked up for sending.", campaign.ToString()));
                    ...
                    Log.Informational(string.Format("Update campaign status to sending {0}", campaign.ToString()));
                    ...
                    //Campaign Recipients
                    Log.Informational(string.Format("Get recipients info {0}", campaign.ToString()));
                    try
                    {
                        ...
                        if (campaignRecipientsInfo.RecipientsInfo.IsAny())
                        {
                            ...
                            try
                            {
                                ...
                                for (int i = 0; i < numberOfIternations; i++)
                                {
                                    ...
                                    try
                                    {
                                        Log.Informational(string.Format("Sending campaign to vmta {0}, iteration {1}", campaign.ToString(), i));
                                        ...
                                        {
                                            Log.Informational(string.Format("Updating invalid email recipients {0}, count {1}", campaign.ToString(), invalidRecipients.Count()));
                                            _campaignService.UpdateCampaignRecipientsStatus(new UpdateCampaignRecipientsStatusRequest
                                            {
                            ...
                            {
                                Log.Critical("Campaign batch failed " + campaign.ToString(), ex);
        ...
        private void SendEmail(int accountId, int userId, int campaignId, bool finalRetry, string campaignName)
        {
            ...
            string email = _userRepository.GetUserPrimaryEmail(userId);
            ...
            })
            {
                ...
                try
                {
                    Log.Informational("Sending mail...");
                    client.Send(message);
                }
                ...
                {
                    Log.Error("Invalid operation exception while sending SMTP error email: ", e);
                }
            }
        }
    }
