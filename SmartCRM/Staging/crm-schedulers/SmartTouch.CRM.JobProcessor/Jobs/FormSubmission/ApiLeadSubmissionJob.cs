    ...
    public class ApiLeadSubmissionJob : BaseJob
    {
        ...
        private Dictionary<string, string> GetContactFields(PersonViewModel model)
        {
            if (model != null)
            {
                ...
                if (model.Emails != null && model.Emails.Any(w => w.IsPrimary && !string.IsNullOrEmpty(w.EmailId)))
        ...
        private PersonViewModel GetUpdatedPersonData(PersonViewModel existingModel, PersonViewModel newModel)
        {
            ...
            Phone mobilePhone = existingPhones.FirstOrDefault(w => w.DropdownValueTypeID == (short)DropdownValueTypes.MobilePhone);
            if (!string.IsNullOrEmpty(mobilePhoneNumber) && !(mobilePhoneNumber.Length < 10 || mobilePhoneNumber.Length > 15))
            {
                string number = mobilePhoneNumber.TrimStart('0', '1');
            ...
            Phone homePhone = existingPhones.FirstOrDefault(w => w.DropdownValueTypeID == (short)DropdownValueTypes.Homephone);
            if (!string.IsNullOrEmpty(homePhoneNumber) && !(homePhoneNumber.Length < 10 || homePhoneNumber.Length > 15))
            {
                string number = homePhoneNumber.TrimStart('0', '1');
                Phone phone = new Phone();
                phone.Number = number;
                phone.AccountID = existingModel.AccountID;
            ...
            if (newModel.CustomFields.IsAny())
            {
                ...
                foreach (ContactCustomFieldMapViewModel submittedField in newModel.CustomFields)
                {
                    try
                    {
                        var isCustomField = accountCustomFields.CustomFields.FirstOrDefault(c => c.FieldId == submittedField.CustomFieldId);
                        if (isCustomField != null)
                        {
                            ...
                            var existingCustomField = existingModel.CustomFields.FirstOrDefault(c => c.CustomFieldId == isCustomField.FieldId);
